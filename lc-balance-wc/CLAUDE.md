...
## BAL-003 "Feature Components + Facade" pilot #2 — `CheckerPanelComponent` extracted, a genuine standalone Angular child component (2026-08-19, user-directed — full 8-phase "Feature Components + Facade" architecture proposal offered against the current `TransactionBuilderComponent`; user chose "全面執行（起點：先做可行性調查）" — commit to the whole direction, starting with a feasibility investigation; investigation confirmed `CheckerActionContext`/`CheckerActionOutcome` are already clean DTOs, materially more favorable than the earlier-declined Look Up Panel investigation, so the user then confirmed "開始實作 CheckerPanelComponent")

Phase 1 of the 8-phase proposal (`TransactionBuilderPageComponent` → `MakerPanelComponent`/`CheckerPanelComponent`/`ReferenceIndexComponent` → `TransactionBuilderFacade` → `TransactionOrchestrationService` → per-function Strategy classes → thin Page Container). This entry covers Phase 1 only — `CheckerPanelComponent`, chosen first per the proposal's own "拆 Maker 與 Checker" priority ordering, since Checker already had "a very clear Component Boundary" (its own independent search/queue/busy/error state, per the proposal's own evidence).

**Scope, narrower than the proposal's own literal description of a full Checker feature owner — a deliberate, disclosed design fork, not an oversight.** Investigated the FULL Checker surface (`checkerLcNumber`/`searchCheckerLc()`/`loadCheckerQueue()`/`onSelectCheckerMovement()` through `checkerAct()`/`release()`/`reject()`/`deleteMakerPending()`/`approveArrival()`) before deciding what moves. Confirmed a real, load-bearing distinction the proposal's own text didn't separate: the SEARCH+QUEUE half (`checkerLcNumber`/`checkerContract`/`checkerItems`/etc.) is genuinely self-contained — its only external dependency is `selectedFunction` (read-only) — while the ACTION half (`checkerAct()`/`release()`/`reject()`/`isCheckerCompoundOwnSubmission`/`checkerActionButtonLabel`) reads DEEP Maker-side context (`submitResult`, `selectedFunctionStrategy`, 4 compound movementIds — already funneled through the existing `CheckerActionContext`/`CheckerActionOutcome` DTOs from an earlier same-session `CheckerActionsService` extraction) and its own "release succeeds → reset the WHOLE Maker screen via `selectFunction()`" side effect is fundamentally a Maker-side concern. Extracting the action half too would have meant either duplicating that whole context surface as further `@Input()`s for no real decoupling gain, or rewriting the ~40 existing compound-release tests in `.actions.spec.ts` (A3S/A6/B4/B5's own multi-leg release chains) that already exercise this exact context/outcome shape end-to-end — judged disproportionate for this pass. `selectedCheckerMovement`/`checkerBusy`/`checkerError`/`checkerId` and every Release/Reject/Approve method **stayed on `TransactionBuilderComponent`**; only the search+queue subsystem moved.

**The genuinely hard design question — how does the PARENT trigger the CHILD's search from ~9 Maker-side call sites (`syncCheckerToContext()`), given this project's own established constraint that `@ViewChild` never resolves under its direct-instantiation (`new Component(...)`, no `TestBed`) test convention?** Investigated and rejected two naive options before landing on the one that shipped:
- **`@ViewChild`** — rejected outright; this is the EXACT blocker the earlier Look Up Panel investigation already found (`@ViewChild` resolution depends on Angular's view-init lifecycle, which direct-instantiation tests never trigger).
- **A plain reactive `@Input()` + `ngOnChanges()`, bound directly to `contextLcNumber`/`contextSecondaryRef`** — investigated and rejected: Angular's `ngOnChanges()` only fires when the bound VALUE differs (by reference, for an object) from the previous change-detection cycle — but the original `syncCheckerToContext()` re-searches UNCONDITIONALLY at every one of its 9 call sites, even when the target LC Number is UNCHANGED from before (e.g. `submitA4()`'s own success callback re-syncs to refresh `checkerItems` after finalizing the SAME already-selected LC's own movement — the LC Number never changes, but the queue's own CONTENTS genuinely need reloading). A value-keyed `@Input()` would silently miss exactly this class of re-sync.
- **What shipped**: a `CheckerSyncSignal` "trigger object" `@Input()` (`{lcNumber, secondaryRef}`) that the PARENT constructs as a BRAND NEW object literal at every one of the 9 call sites — reference INEQUALITY, not value equality, is the signal, so `ngOnChanges()` reliably re-fires even when the contained values are identical to the previous call. A companion `resetTrigger: number | null` (a plain incrementing counter, `checkerResetNonce`) handles `selectFunction()`'s own per-function reset the same way. Both are genuine, directly-testable `ngOnChanges()` triggers — `ngOnChanges({...})` is a plain method call, no `TestBed`/view-init lifecycle needed, verified directly in `checker-panel.component.spec.ts`.
- **A second real risk found and specifically avoided while designing this**: `selectedCheckerMovement`/`checkerError` are STILL parent-owned fields (per the scope decision above) — if `selectFunction()`'s own reset had relied on a round-trip through the child's `ngOnChanges()` to clear them (child processes `resetTrigger` → emits an event → parent's handler clears its own mirror), this would silently break under this project's own direct-instantiation Jest tests, since `ngOnChanges()` never fires there — meaning `selectFunction()` would appear to reset these two fields in the REAL app (where change detection runs) but NOT under test, a correctness/test-divergence risk. Fixed by having `selectFunction()` clear `selectedCheckerMovement`/`checkerError` DIRECTLY and unconditionally (same as before extraction), with `checkerResetNonce++` firing PURELY as a "tell the child to clear its own internal display state" signal that the child's own reset never needs to round-trip back to the parent for.

**Content projection for the visual "one card" requirement.** The Checker section's own markup was always ONE `<section class="tb-section tb-section--checker">` — the search+queue half now lives in `checker-panel.component.html`, the STILL-parent-owned Release/Reject action block (Checker ID input, the deferSettlement acknowledgment hint, the buttons, `checkerError`) is projected INTO it via `<ng-content>`, placed inside the SAME `*ngIf="checkerContractId"` gate the child's own picker uses — preserving the exact original visibility condition (the actions block only ever showed once a contract was resolved) without a second, visually-separate card.

**Outputs, each mapped to one small parent handler, mirroring an ORIGINAL inline reset point exactly:**
- `movementPicked: BalanceMovement | null` — fired by a genuine user pick (`onSelectCheckerMovement()`) AND by the top-of-method resets inside `searchCheckerLc()`/`loadCheckerQueue()` (mirrors the original's own unconditional `this.selectedCheckerMovement = null` at both those points). Parent's `onCheckerMovementPicked()` mirrors the value AND resets `arrivalApproved` — a deliberate, harmless simplification over the ORIGINAL narrower scope (only `onSelectCheckerMovement()` used to reset `arrivalApproved`): every template condition reading `arrivalApproved` already requires `selectedCheckerMovement` non-null too, which an implicit reset always clears in the same breath, so this has zero observable behavior difference — documented in the method's own doc comment rather than silently introduced.
- `queueReloaded` — fired at the top of every `loadCheckerQueue()` run; parent's `onCheckerQueueReloaded()` clears its own `checkerError` (mirrors the original's own unconditional `checkerError = null` there — genuinely necessary, not just for symmetry, since a stale Release/Reject error would otherwise linger visibly across a fresh reload).
- `queueLoadSucceeded` — fired once `listMovements` succeeds; parent's `onCheckerQueueLoadSucceeded()` calls the still-private `syncLookupToContext()`, mirroring the original inline success callback exactly.

**Verification:**
- `npx tsc -p tsconfig.app.json --noEmit` clean; `ng build --configuration development` (strict templates — validates every new `[input]`/`(output)`/`<ng-content>` binding) clean.
- `npm run lint` — 0 errors, 216 warnings (down from the prior 221-228 baseline — no new `any` usage, some removed).
- `npx prettier --check`/`--write` clean on all touched/new files.
- Full Angular Jest suite: **923/923 passing** (up from the prior 880/880 baseline — 22 new tests in the new `checker-panel.component.spec.ts`, plus several existing tests in `transaction-builder.component.spec.ts`/`.selection.spec.ts`/`.gaps.spec.ts`/`.actions.spec.ts` migrated in place, not mechanically renamed, since the state owner genuinely changed class — each migrated test reproduces its ORIGINAL assertion's own intent against the new boundary: `searchCheckerLc()`'s whole describe block moved to the new file verbatim in spirit; `checkerContractId`/`checkerSecondaryField`/`checkerSecondaryLabel`'s own component-level delegation test moved too (the pure-function logic itself was already independently covered by `function-policy.spec.ts`, confirmed before deleting); the "resets prior selection state" test's own Checker-specific poison/assert lines were replaced with a `checkerResetNonce` increment assertion). Coverage **99.12/96.76/97.7/99.28%** — all four metrics clear the 95% floor, `checker-panel.component.ts` itself at **100/100/100/100**.
- `backend/` **34/34** and microservice **335/335**, both unaffected (Angular-only change — no request/response contract touched).
- **Live-verified end to end against the real running stack** (already-running `ng serve`/backend/microservice from this same long session — connected directly per this project's own "don't chase port conflicts" posture; watch mode picked up every source change automatically): (1) fresh page load — zero console errors, confirming no `NullInjectorError`-class regression (the exact failure mode the earlier same-day F-04 incident taught this session to always check for before calling a DI/component-wiring change done); (2) picked A1, `<app-checker-panel>` rendered its own header + search box correctly; (3) submitted a fresh A1 (`CHKPILOT01`, 50,000 USD) — the Checker panel **auto-synced to the new LC with zero explicit search**, header correctly read "Pending Approvals — LC CHKPILOT01" and the queue correctly showed the just-created PENDING ISSUE row — directly proving the `CheckerSyncSignal`/`ngOnChanges()` cross-component trigger works in the real, change-detection-driven app, not just in the unit tests; (4) picked the PENDING row — the projected Release/Reject action block correctly appeared (proving `onCheckerMovementPicked()`'s mirror + the shared `*ngIf="checkerContractId"` gate both wired correctly); (5) clicked Release — succeeded with zero console errors, the movement left PENDING; (6) switched to A2 — the Checker panel's own header correctly reverted to plain "Pending Approvals" (no LC suffix), directly proving `checkerResetNonce`/`resetTrigger`/`resetPanel()` also fires correctly in the real app (an initial read taken immediately after the Release click had still shown the stale LC suffix — traced to a timing artifact of reading the accessibility tree before that async subscribe callback's change detection had fully propagated, not a real defect, confirmed by this independent A1→A2 switch exercising the identical reset code path cleanly). Test data (`CHKPILOT01`) cleaned up afterward via a direct SQLite delete; 47 pre-existing reference contracts confirmed untouched.

**Net effect on the 8-phase proposal**: Phase 1 (`CheckerPanelComponent`) is done, at the scope this pass's own investigation confirmed was safely extractable. `transaction-builder.component.ts` line count is roughly flat (fields/methods removed roughly offset by new signal-construction/doc-comment lines — never a line-count exercise, same posture every prior extraction in this file has taken) — the real gain is a second proof point (after `IndexPickerComponent`/`AccountEntriesDialogComponent`) that a genuine `@Component` boundary is achievable under this project's own test convention, this time for a component with EXTERNALLY-triggered refresh needs, which neither prior pilot had to solve. Phases 2-8 (MakerPanelComponent, ReferenceIndexComponent, the Facade, Orchestration Service, per-function Strategy classes, the thin Page Container) are **not started** — each is its own separately-scoped decision, not assumed to follow automatically from this one's success.

Not committed (not requested).

## BAL-003 "Feature Components + Facade" pilot #2, Phase 2 — `MakerPanelComponent` extracted, the largest and riskiest piece of the 8-phase proposal (2026-08-19/20, user-directed — "commit and push then Phase 2", confirmed full scope, not a narrower slice, after being warned it was materially riskier than Phase 1: "3 做好測試 與BROWSER測試" — do it properly, with solid test coverage AND browser testing)

Phase 2 of the 8-phase "Feature Components + Facade" proposal (Phase 1, `CheckerPanelComponent`, is the entry immediately above). This work was interrupted mid-flight by a connection drop after Phase 2's three new files (`maker-panel.component.ts/.html/.scss`) already existed but the parent (`transaction-builder.component.ts/.html`) was still untouched — a coordinator resumption message picked it up from that checkpoint and carried it through to completion; the interruption itself changed nothing about scope or approach, just split the work across two sessions of continuous context.

**What moved**: everything Maker-side — the natural-key search (`model`/`naturalKey`/`searchNaturalKey`), the 3 paginated pickers' own selection handlers (`onSelectContract`/`onSelectParent`/`onSelectIbIndex`/the arrival-SG and payable-movement pickers, via the already-extracted `CatalogPickerService`/`PickerSelectionService`/`DocumentArrivalHintsService`), the Formly `fields`/`submit()`/`submitA4()` dispatch across all 14 named business functions, `submitResult`/`submitError`/the 5 compound-leg movement fields, and every Maker-only getter (`formLocked`/`isSubmitReady`/`displayFields`/etc.) — into a new standalone `MakerPanelComponent`. `TransactionBuilderComponent` keeps Checker-side state (`selectedCheckerMovement`/`checkerBusy`/`checkerError`/`checkerAct()`/`release()`/`reject()`/`deleteMakerPending()`/`isCheckerCompoundOwnSubmission`) plus a small read-only mirror of Maker state (`makerContext: MakerCheckerContext`, kept current via a `contextChanged` `@Output()`) that Checker-side release/reject logic needs to build its own `CheckerActionContext`.

**Design — the shared-mutable-state problem, and why it needed two separate signal channels, not one.** Both Maker-initiated Submit and Checker-initiated Release/Reject write into the same conceptual state (`submitResult`/the compound-leg fields) via genuinely different code paths — `applyMakerSubmitOutcome()` on the Maker side, `applyCheckerActionOutcome()`-equivalent logic on the Checker side. Resolved via the same "fresh-object-per-emission `@Input()` + `ngOnChanges()`" mechanism Phase 1's own `CheckerSyncSignal` already established: `MakerPanelComponent.externalCheckerOutcome: CheckerActionOutcome | null` (parent forwards every `release()`/`reject()`/`deleteMakerPending()` outcome except the special whole-screen-reset case), handled by a new `applyCheckerOutcome()` on the child that mirrors the original monolithic component's own `applyCheckerActionOutcome()`/`finishCheckerAction()`/`failCheckerAction()` logic exactly, now writing into child-owned state instead.

**A second, narrower signal was required for one specific reason, found and fixed BEFORE any verification ran (a self-caught design error, not a live-caught one)**: `checkerAct()`'s PLAIN (non-compound, direct `api.release()/api.reject()`) success path — re-derived from the original pre-extraction source rather than assumed — never touches `submitResult` at all; it only refreshes the snapshot and re-syncs. A Checker acting on an item found via their own independent search may have nothing to do with whatever the Maker currently has selected, so routing this outcome through the same `externalCheckerOutcome` channel would have silently corrupted the Maker Result panel for an action the Maker may never even be aware of. First draft did exactly this (routed it through `externalCheckerOutcome`) — caught by re-reading the original `checkerAct()` body line-by-line before running anything, fixed with a second, narrower `refreshRequested: number | null` `@Input()` (a plain nonce, handled by `ngOnChanges()` calling ONLY `refreshSelectedContractSnapshot()`/`emitSync()`, never touching `submitResult`) and a matching `refreshNonce` counter on the parent.

**A second self-caught error, same pass, same "re-derive before running anything" discipline**: `MakerPanelComponent`'s first draft had no `providers` array at all despite constructor-injecting three services with no `@Injectable()` decorator of their own (`CatalogPickerService`×3, via `InjectionToken`s) plus two plain `@Injectable()`-no-`providedIn` services — exactly the `NullInjectorError` failure class this whole session's own F-04 saga (see the entries above) had already hit and fixed once for the parent. Caught by grepping every injected service's own decorator status before running `ng build`, fixed by mirroring the parent's own already-proven `providers` array pattern (bare class-type entries for the two plain services, `{provide: TOKEN, useFactory: () => new CatalogPickerService(SIZE, inject(BalanceComponentApiService))}` for the three page-sized instances) before any verification ran, not after a crash.

**A live-browser-caught bug, NOT caught by any static check or the 927-test suite — the actual reason this project's own "always verify DI/component-wiring changes live" rule exists.** `MakerPanelComponent.ngOnChanges()` mirrored `CheckerPanelComponent`'s own `resetTrigger` handling verbatim — `if (changes['resetTrigger'] && !changes['resetTrigger'].firstChange) this.resetForFunction();` — correct for `CheckerPanelComponent` (whose class-field-initializer defaults already ARE its own reset state, so skipping the very first firing is harmless) but WRONG for `MakerPanelComponent`: `<app-maker-panel>` only ever gets created the FIRST time a function is picked (it lives inside `*ngIf="selectedFunction"`), so the very first `resetTrigger` change is ALWAYS `firstChange: true` — meaning `resetForFunction()` (which calls `rebuildFields()`, the only thing that ever populates the Formly `fields` array) never ran on a Maker's very first function pick. Live-tested in the browser (per the mandatory-verification checklist): picking A1 for the very first time on a fresh page load rendered a MAKER card with a header and nothing else — zero form fields, not even the LC Number input. Picking A2 immediately after (a SECOND function-switch, `firstChange: false`) rendered correctly, confirming the diagnosis precisely: every function's FIRST pick, on a fresh page load or after returning from "no function selected," would have shipped completely broken, while every subsequent switch worked fine — the kind of bug a quick manual click-through easily glosses over (a tester who picks 2+ functions in a row, which is the natural way to explore an app, never sees the failure) but a fresh end-user hitting the page for the first time always would have. Neither `tsc --noEmit`, nor the strict-template `ng build`, nor the 927-test Jest suite (whose own migrated tests happened to always call `ngOnChanges({resetTrigger: ...})` in a way that never isolated the true first-fire case) caught this. **Fix**: `if (changes['resetTrigger']) this.resetForFunction();` — unconditional, matching what the ORIGINAL monolithic `selectFunction()` always did (called its full reset stanza inline on every single pick, first or not, since there was no `ngOnChanges()`-firstChange concept in a non-extracted component). Re-verified live immediately after the fix: the exact same first-A1-pick-on-a-fresh-page scenario now renders the full Formly form (LC Number/Amount/Currency/Tolerance/Tenor Type/Submit A1) correctly.

**Also caught in this same static-verification pass, before any live check** (strict-template `ng build` errors `tsc --noEmit` alone is structurally blind to, per this project's own repeated lesson): the parent's own `imports` array was missing `FormlyModule`/`IndexPickerComponent` — both still needed by the Look Up / Inquire Events sections that stayed on the parent — and `maker-panel.component.html` had 15 `selectedFunction.xxx` accesses needing `?.` once the input's real nullable type was strict-template-checked (the child never had the parent's own narrowing `*ngIf="selectedFunction"` wrapper reach across the component boundary); and `releaseSuccessHint` (the brief post-Release confirmation text, correctly parent-owned since `release()`'s own screen-reset case sets it) needed a new `@Input()` on the child since the markup displaying it moved there — all three fixed and re-verified via `ng build` before the live browser pass.

**Verification**:
- `npx tsc -p tsconfig.app.json --noEmit` clean; `ng build --configuration development` (strict templates) clean, after the fixes above.
- `npm run lint` — 0 errors, 216 warnings (baseline, unchanged).
- `npx prettier --write`/`--check` clean on every touched/created file.
- **The 267+ Maker-owned test assertions across 4 spec files migrated into a new `maker-panel.component.spec.ts`** (3803 lines), constructed via `new MakerPanelComponent(mockApi)` — same no-TestBed convention as every other spec file in this project. `transaction-builder.component.spec.ts`/`.actions.spec.ts`/`.gaps.spec.ts` trimmed to their now-genuinely-parent-owned surface (Checker actions, `checkerAct()`, `release()`/`reject()`/`deleteMakerPending()`, the `makerContext` mirror, `onMakerSyncRequested()`); `transaction-builder.component.selection.spec.ts` (entirely Maker-owned content) deleted outright, its content having moved wholesale. Every place a test used to poke `comp.submitResult`/`comp.selectedPayMovement`/the compound movementIds directly on the parent now builds `(comp as any).makerContext = {...}` — the same established test-only-private-access convention this codebase already uses elsewhere. One non-obvious finding surfaced during migration: `TransactionBuilderComponent` has **no `submitError` field at all** any more — Checker-action failures route through `makerOutcomeSignal = {kind:'failed', message}` instead, so every old `expect(comp.submitError).toBe(...)` assertion in `release()`/`reject()`/`deleteMakerPending()` was converted to `expect(comp.makerOutcomeSignal).toEqual({kind:'failed', message: ...})`.
- Full Angular suite: **927/927 passing** (up from 568/four-suites-failed-to-compile immediately after the structural rewrite, before migration), coverage **97.91/96.01/97.04/98.01%** — all four metrics clear the 95% floor (`maker-panel.component.ts` itself 94.95/96.56/93.93/94.74 — the uncovered lines are the `providers` array's own `useFactory` closures, never invoked by this project's own direct-instantiation tests, same class of gap this file already accepts for every prior extraction's own DI-wiring code).
- `backend/` **34/34** and microservice **335/335**, both unaffected and re-run per this file's own standing three-suite rule (Angular-only change).
- **Live-verified end to end against the real running stack**, per the mandatory multi-path checklist (fresh page load / a plain function full cycle / a compound function full cycle / a validation-failure path):
  1. **Fresh page load, first-ever function pick** — zero console errors; caught the `ngOnChanges` firstChange bug above; re-verified clean after the fix.
  2. **A1 (LC Issue) full Submit → Release cycle** (`PHASE2TEST01`, 50,000 USD Sight) — Submit correctly produced a MAKER RESULT panel (Status: PENDING, Account Entries, Delete Pending (EC)); the CHECKER panel **auto-synced to the new LC with zero explicit search**, directly proving the `onMakerSyncRequested()`/`checkerSyncSignal` cross-component wiring fires correctly in the real, change-detection-driven app (not just under the migrated unit tests); Look Up Current Balance auto-synced too (Available 50000/Pending Earmark 50000). Release succeeded — Event Timeline correctly flipped to `APPROVED`/`50000`, Confirmed Balance `50000`. Confirmed via a direct git-history comparison (commit `26d25e1`, the last commit before this session's Phase 2 work) that a PLAIN function's Checker Release never auto-resets the screen (only the compound-release paths do, via `isCheckerCompoundOwnSubmission`) — the observed post-Release state (Maker Result panel unchanged) matches this exactly, not a regression.
  3. **A8 (SG Issue, 20,000 against the same LC) → Release**, then **A3S (Document Arrival w/ Shipping Gtee) full compound Submit → Release** — the eligibility-filtered LC Index correctly showed `PHASE2TEST01` only once its SG existed; the SG picker auto-selected its own sole candidate (`PickerSelectionService` correctly wired); Submit correctly produced BOTH "Account Entries" and "Account Entries — SG Redemption" buttons (proving `outcome.secondary` flows through `MakerSubmitService` → `applyMakerSubmitOutcome()` → `arrivalSgRedeemMovement` correctly) and dropped Off-Balance Exposure to 0 (PENDING SG redemption netting out per §6.1); the Checker's single Release correctly did BOTH — the SG genuinely `RELEASED` (confirmed: it dropped out of A3S's own eligible-SG list on the next render) while the LC's own UTILIZE correctly stayed `PENDING`/`EARMARKING` (A3S's own documented "not yet finalized — go to A4/A6 next" semantics) — directly exercising `isCheckerCompoundOwnSubmission`'s `businessEventId`-based routing and `forwardOutcomeToMaker()`.
  4. **Validation-failure path, A2 (LC Amendment)** — picked the function, selected Direction, picked the LC, typed an Amount but left the mandatory Amendment No. blank: Submit A2 was correctly a no-op (the 2026-08-19 "Submit Button Enablement" gate — `isSubmitReady` — correctly kept it disabled), the form stayed fully editable, no error state leaked. Filled in Amendment No. and re-submitted — succeeded cleanly (`AMEND_INCREASE 5000 A01 PENDING`), confirming the gate unlocks correctly once every mandatory field is genuinely valid, and that switching through 4 different functions (A1→A8→A3S→A2) in one continuous session never leaked stale state between them.
  - Zero console errors across the entire live session, checked directly via `read_console_messages`, not just the absence of a visible error banner.
  - Test data (`PHASE2TEST01` + its SG `G01`) scoped-cleaned afterward via a direct SQLite delete; the pre-existing 47 reference contracts confirmed untouched.

**Scope narrowing versus the original hypothesis**: none — the full Maker-side surface (natural-key search, all 3 pickers' selection handlers, all 14 functions' Submit dispatch, all 5 compound-leg fields) moved as originally scoped, matching the user's own explicit "3 做好測試 與BROWSER測試" (do it fully, with real test coverage and real browser testing) confirmation rather than a reduced slice.

**Net effect on the 8-phase proposal**: Phases 1 (`CheckerPanelComponent`) and 2 (`MakerPanelComponent`) are both done. `transaction-builder.component.ts` shrank from ~2,689 lines (pre-Phase-2, post-F-04) to a genuinely small orchestration/wiring layer — function/side selection, the Checker action layer, the `makerContext` mirror, and the Look Up/Inquire Events template sections it still owns directly. Phases 3-8 (a unified Primary/Secondary Index Component, the `TransactionBuilderFacade`, `TransactionOrchestrationService`, per-function Strategy classes taking over function-specific behavior, and finally shrinking `TransactionBuilderComponent` itself into a thin page container) remain **not started** — each is its own separately-scoped decision per the proposal's own recommended sequencing, not assumed to follow automatically from Phase 2's success.

Not committed (not requested).

## BAL-003 "Feature Components + Facade" pilot #2, Phase 3 — eligibility-filtering logic unified across the 3 picker getters (2026-08-20, user-directed — reopened after Phase 2 shipped; user first said "跳過 Phase 3，直接進 Phase 4" (skip Phase 3, go to Phase 4) in response to a feasibility check showing Phase 3's core goal — a unified Primary/Secondary Index Component — was already achieved via `IndexPickerComponent`/`CatalogPickerService`/`PagedListState`, but then mid-turn corrected to "Phase3 做完其他再來分析" (finish Phase 3 first, then analyze the rest); a follow-up `AskUserQuestion` clarified this meant the ONE genuine remaining gap — unifying the 3 `filteredXxxCatalog` getters' own eligibility-rule cascades, previously evaluated during the `CatalogPickerService` pagination pass and judged not worth the risk/reward at the time — and the user confirmed "實作 eligibility 邏輯統一（接受風險）", explicitly overriding that earlier call)

Scope, precisely: `filteredCatalogContracts`/`filteredParentCatalog`/`filteredIbIndexCatalog` (`maker-panel.component.ts`, moved here from the parent during Phase 2) each independently hand-rolled the identical shape — an optional tenor pre-filter, a cascade of function-specific eligibility checks (each an early-return `list.filter(c => hintSet.has(c.balanceContractId))`), an unconditional-bypass case for some functions (parent picker only), and a trailing "exclude 0-Available-Balance candidates" generic fallback.

**Design landed on**: a new `eligibility-rule.ts` — a small, pure, DI-free module (`EligibilityRule` discriminated union + `applyEligibilityRule()`), matching this codebase's own established convention for shared picker-adjacent utility logic (`paged-list-state.ts`'s own precedent) rather than a stateful service. Deliberately unifies ONLY the mechanical TAIL (apply an already-resolved rule), not the decision of WHICH rule applies to a given function/picker — that decision stays local to each of 3 new private resolver methods on `MakerPanelComponent` (`resolveCatalogEligibilityRule()`/`resolveParentEligibilityRule()`/`resolveIbIndexEligibilityRule()`), unchanged in branch order/conditions from before. Two reasons this boundary was chosen over folding the rule-resolution itself into `function-strategy.ts`'s existing Strategy pattern (the design direction originally proposed): (1) two of the branch conditions read raw `TransactionFunction` registry fields (`payableMovementInstrumentType`, `catalogTenorFilter`) that were explicitly scoped OUT of the F-01 Strategy migration — forcing them into `FunctionStrategy` now would either grow that file past its own documented scope for one caller, or duplicate those fields into a second place; (2) no function ever needs its catalog-picker and parent-picker eligibility to differ (each function uses exactly one Step-1 picker type, decided by `hasParent`), so a discriminated union keyed by picker was unnecessary — one `EligibilityRule` shape genuinely serves all three call sites.

**A real, subtle bug caught by the test suite, not by static checks** — exactly the class of mistake this finding's own earlier risk assessment worried about: the first implementation pass merged all three getters' trailing "generic fallback" branch into one identical case, gated by `DECREASING_MOVEMENT_TYPES.has(movementType)`. `filteredCatalogContracts`/`filteredIbIndexCatalog`'s own original trailing branch WAS gated this way (an Increase/Create never excludes a 0-balance candidate — 0 is a normal starting point). `filteredParentCatalog`'s own original trailing branch was **not** — it applied the 0-balance exclusion unconditionally, with no `DECREASING_MOVEMENT_TYPES` check at all (confirmed via `git show HEAD:...` against the pre-unification source, not assumed). Merging them into one gated shape silently changed A8's (Shipping Gtee Issue, `movementType: 'ISSUE'`, not itself a decreasing type) own Parent LC picker from "always exclude 0-balance candidates" to "never exclude them" — caught immediately by the pre-existing `filteredParentCatalog: no tenor flags at all (A8) applies only the 0-balance filter` test, which failed with the wrong candidate set. Fixed by adding `gatedByMovementType: boolean` to the `EligibilityRule`'s own `genericFallback` variant — `true` for catalog/IB Index, `false` for parent — preserving the asymmetry exactly rather than "fixing" it (out of scope; zero business logic change is the whole point of this pass). `eligibility-rule.ts`'s own doc comment documents this asymmetry explicitly for the next reader, since nothing else in the codebase previously called it out.

**Verified**: `npx tsc -p tsconfig.app.json --noEmit`/`ng build --configuration development` (strict templates)/`npm run lint` (0 errors, 228 warnings, unchanged) all clean, `npx prettier --write` applied to both touched files (whitespace-only, re-verified via a clean `tsc --noEmit`/full-suite re-run afterward). Full Angular suite **927/927 passing** (unchanged count — pure structural unification, no new test needed beyond what already existed; `eligibility-rule.ts` itself lands at **100/100/100/100** coverage purely from the existing 3 getters' own test coverage exercising it indirectly), coverage 97.91/96/97.03/98.02% (all four clear the 95% floor). `backend/` 34/34 and microservice 335/335 both re-run per this file's own standing rule, unaffected (Angular-only change, no request/response contract touched).

**Live-verified end to end against the real running stack, covering all 4 distinct eligibility paths named in this pass's own scope** (built `BROWSERELIG1`: A1 Issue 50,000 → Released → A3 Document Arrival 20,000/B01, left PENDING — EARMARKING; `BROWSERELIG2`: A1 Issue 40,000 → Released, no Document Arrival, then A8 SG Issue 10,000/GBROWSER01 against it, left PENDING):
- **A4's own LC Index** (`catalogPayableIbs`, `resolveCatalogEligibilityRule()`'s first branch) — correctly showed only `BROWSERELIG1 — B01` (1 of the 14 candidate LCs), `BROWSERELIG2` (no outstanding Document Arrival) correctly excluded.
- **A3S's own LC Index** (`catalogSgEligible`) — correctly showed only `BROWSERELIG2` (1 total), `BROWSERELIG1` (no SG) correctly excluded.
- **A9's own Parent LC picker** (`parentSgEligible`, the parent-side mirror of the same rule) — correctly showed only `BROWSERELIG2` (1 total), confirming the SAME semantic rule resolves correctly on both the catalog AND parent pickers independently.
- **A8's own Parent LC picker** (the bug this pass's own test suite caught, `gatedByMovementType: false`) — correctly showed BOTH `BROWSERELIG1 (SIGHT)` and `BROWSERELIG2 (SIGHT)` among the 14 ACTIVE candidates (neither wrongly excluded), confirming the unconditional-fallback shape survived the fix.
- **A2's own LC Index with Direction=Decrease** (`AMEND_DECREASE`, in `DECREASING_MOVEMENT_TYPES` — the gated catalog fallback genuinely gated, not bypassed) — correctly showed both `BROWSERELIG1`/`BROWSERELIG2` (5 total incl. pre-existing reference LCs), neither excluded since neither has a 0 Available Balance.

Zero console errors observed across the entire session. Test data (`BROWSERELIG1`/`BROWSERELIG2`/the `GBROWSER01` SG) cleaned up afterward via a direct SQLite delete; the pre-existing 47 reference contracts confirmed untouched.

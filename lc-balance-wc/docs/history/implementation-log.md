...
Angular 與 Web Component host 共用相同 Maker Panel state machine。新增 A7 regression test，完整 WC 測試為 62 suites／1,732 tests。HTTP request／response schema、endpoint 與 status code均未變更，因此本資料夾的 microservice 與 channel OAS 分別驗證、內容不修改；DOM event/method contract 亦不變。

Maker Submit 隨後套用相同的 raw-cause preservation 原則：`MakerSubmitOutcome`、pure workflow reducer、共用 Maker Panel 與 Maker Result presenter 現在保留原始 HTTP error。Angular／Web Component host 的 HTTP 500 不再誤判成 `BAL-UI-UNEXPECTED`；成功 Submit 則明確清除 error 與 cause。HTTP OAS 與 DOM contract 均不變。

## 2026-08-31 — Angular／Web Component safe-read automatic retry policy

新增共享 HTTP interceptor，Angular host 與 `<balance-component-app>` 使用相同 policy：僅對 GET／HEAD／OPTIONS 的 network/status 0、408、429、5xx 暫時性失敗自動重試。`.env` 的 retry count／initial delay／maximum delay 預設為 3／250ms／2000ms，build/start/test 前產生型別安全設定；POST command 不自動重送，避免重複 balance movement 或 Account Entries。

本資料夾的 microservice OAS 獨立升至 v1.42.1，以 `x-client-retry-policy` 記錄 client operational policy；HTTP wire contract及 Web Component DOM contract 均未改變。

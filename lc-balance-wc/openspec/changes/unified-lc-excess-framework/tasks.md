...
- [ ] 8.4a Prepare a reproducible native ARM64 SonarQube environment using `../lc-ssi-wc/qa/docker/sonarqube.compose.yml` and `sonar-scanner.Dockerfile` as the reviewed CLI／Docker reference. Adapt rather than copy it: run PostgreSQL plus SonarQube `26.9.0.129388-community` and the multi-stage scanner as `linux/arm64`; replace the reference scanner's `latest` base with an explicit version and verified immutable digest; record the PostgreSQL／SonarQube／scanner image digests plus host and container architecture evidence. Mount source read-only, keep scanner work under tmpfs, use project key `lc-balance-wc`, and set `sonar.qualitygate.wait=true`; the reference's `lc-ssi-wc` key and `false` wait setting are forbidden.
- [ ] 8.4b Create an isolated clean checkout／worktree at the exact `OVERDRAWN` release-candidate SHA before coverage or scan. Retain `git rev-parse HEAD`, an empty `git status --porcelain`, and source-tree identity evidence. In that checkout, run and retain the successful commands that freshly generate all three configured LCOV files; record each LCOV path, completion timestamp and SHA-256, and retain scanner log evidence that those exact paths were imported after generation.
- [ ] 8.4c Run the scan with `sonar.projectVersion` equal to the verified release-candidate SHA and require both scanner success and Compute Engine task `SUCCESS`. Export and retain the New Code Definition／baseline and period, CE task and analysis ID, project analysis version／time, Quality Gate status by analysis ID, overall and `new_*` measures, issues by severity／type／New Code status, and reviewed Security Hotspots using the corresponding SonarQube APIs. Evidence MUST bind project key, analysis ID, version, commit SHA, clean-tree proof and LCOV hashes; a pre-candidate, stale, different-project, different-tree or different-commit scan is invalid.
- [ ] 8.4d Pass the approved Quality Gate. New Code MUST have Issues = 0, Security Hotspots Reviewed = 100%, Coverage >= 92%, Duplicated Lines <= 1%, Maintainability Issues = 0, Medium Severity Issues = 0 and Security Issues = 0. Overall Code MUST have Coverage >= 92%, Duplicated Lines <= 3%, High Severity Issues = 0, Maintainability Issues <= 20, Medium Severity Issues = 0, Security Hotspots Reviewed = 100% and Security Issues = 0. Inject an ephemeral database password and project-scoped scan token without placing either secret in tracked or untracked `.env` files, compose files, scripts, command arguments, logs or exported evidence; do not dump the environment, redact evidence, and revoke the token after evidence export.

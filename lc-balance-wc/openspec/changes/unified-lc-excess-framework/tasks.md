...
- [ ] 8.4a Run a fresh SonarQube server scan against the exact `OVERDRAWN` release-candidate commit and retain machine-readable measures／Quality Gate evidence. Use `../lc-ssi-wc/qa/docker/sonarqube.compose.yml` and `sonar-scanner.Dockerfile` as the reviewed CLI／Docker reference: run PostgreSQL plus SonarQube `26.9.0.129388-community` and the multi-stage scanner natively as `linux/arm64`, mount `lc-balance-wc` source read-only, keep scanner work under tmpfs, and supply database password／project token only through environment variables. Adapt rather than copy the reference: project key MUST be `lc-balance-wc`, project version MUST identify the exact `OVERDRAWN` commit SHA, and `sonar.qualitygate.wait` MUST be `true` (the reference's `lc-ssi-wc` key and `false` wait setting are forbidden). Regenerate all three LCOV inputs immediately before scanning; require scanner／compute-engine success, then export Quality Gate status and measures through the SonarQube API with analysis ID／time／project version／commit identity. New Code MUST have Issues = 0, Security Hotspots Reviewed = 100%, Coverage >= 92%, Duplicated Lines <= 1%, Maintainability Issues = 0, Medium Severity Issues = 0 and Security Issues = 0. Overall Code MUST have Coverage >= 92%, Duplicated Lines <= 3%, High Severity Issues = 0, Maintainability Issues <= 20, Medium Severity Issues = 0, Security Hotspots Reviewed = 100% and Security Issues = 0. A pre-candidate, stale, different-project or different-commit scan is not evidence.

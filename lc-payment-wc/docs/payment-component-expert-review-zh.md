...
**状态：已实现——直接移除容差（reviewer-confirmed）。** 没有把容差与 FX 腿数挂钩，而是让 Confirm 流程**对所有 originModule（含 RPFM）一律要求精确 Dr = Cr**（`options.balanceTolerance ?? 0`）。总账凭证必须精确平衡；那个 ±0.01 本是屏幕层百分比拆分的 slack（`Debit_Chk_Total_Pct` / `CHK_Total_Pct`），不是过账规则，且 FSD 自己验证的场景（§13.2）本就精确平（差额 0）。`RPFM_BALANCE_TOLERANCE` 仅保留供参考、不再自动套用；`balanceTolerance` 仍作为显式的每次调用逃生口。

**后续项（未实现）——舍入差额腿。** 既然要求精确平衡，任何确实会产生"半个最小单位"级舍入残差的流程（如多腿 FX 拆分），都必须**把残差以一条真实腿显式过到指定的"舍入差额"总账科目**，让凭证既**精确平**、残差又**可审计**——绝不用容差吞掉。这个生成步骤尚未实现；在去掉 ±0.01 之后，它才是真实舍入残差的正确归宿。

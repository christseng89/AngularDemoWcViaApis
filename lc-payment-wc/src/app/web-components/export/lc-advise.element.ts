...
export class LcExportAdviseElement extends HTMLElement {
  private _form = {
    applicantId: 'C-001',
    lcAmount:    500000,
    lcCurrency:  'USD',
    // No advRate — flat TWD 500 commission
  };
  ...
  private _initCharges() {
    ...
    // Advising commission: default TWD (local collection), can also pay in LC currency
    this._charges = [
      { id: 'comm', label: '通知費 Advising Commission', amtTwd: Number(s['advCommTwd']), ccy: 'TWD', ccyOptions: ['TWD', ccy] },
    ];
  }
  ...
  private _draw(): void {
    ...
    <div class="card">
      ...
      <form id="form">
        ...
        <button class="btn" type="submit" ${this._loading ? 'disabled' : ''}>${this._loading ? 'Calculating…' : 'Calculate Advising Commission'}</button>
      </form>
      ...
      ${s ? `<div class="result-section">
        <div class="sum-card">
          <div class="sum-title">Advising Summary</div>
          <div class="sum-grid">
            ...
            <hr class="sep"/>
            <span class="sl total-lbl">Advising Commission (Flat)</span><span class="sv total-val">TWD ${fmt(Number(s['advCommTwd']), 'TWD')}</span>
          </div>
        </div>
  ...
  private async _calculate(fromChargeChange = false): Promise<void> {
    ...
    try {
      ...
      const res = await fetch('/api/export/advise/calc', {
...
customElements.define('lc-export-advise', LcExportAdviseElement);

...
### 5.1（中）「仍待取得之卷冊」那一列被刪除了

v4 的 `SR2025 vs SR2026` 工作表有一列：

> 仍待取得之卷冊：Category 1（MT101/102/103/104/107）與 Category 4／7（MT4xx／MT7xx）之 MRG 尚未取得。

v5 把它換成了「仍待取得之受控文件」，內容改為 Additional NVR、MyStandards UG、Translation Portal rules——**全部是 MX stack 的項目，Category 1／4／7 的 MRG 缺口消失了。**

我理解本文件已聚焦 MT2XX，但 `仍待取得` 這類欄位存在的目的，正是防止讀者把覆蓋範圍讀得比實際更完整。**MT4xx／MT7xx 才是本專案的主戰場**，把這個缺口從文件裡拿掉，對 Gate 審查者是不利的訊號。建議加回，並明確標為「本文件範圍外，但為專案整體之已知缺口」。
...
| 構面 | 權重 | v4 | v5 | 說明 |
|---|---:|---:|---:|---|
| SWIFT MRG 正確性 | 30 | 27 | 29 | COV option 集合補回並加對比說明；MT205／205 COV 無 54a 補回；C06 責任歸屬且區分 FIN 與 MX 驗證。本次未發現 MRG 層面錯誤 |
| ISO 20022 對應正確性 | 20 | 19 | 19 | pacs.010 之 CBPR+ 範圍已以 CR3035 關閉並經我獨立查證；Cover MPG 版本自行更正。扣分：CR3035 之操作性後果（Margin Collection UG 退場）未取用 |
| 範疇與治理決策 | 20 | 19 | 20 | `transportFormat` vs `displayFormat` 的型別化拆分；legacy atomic fail-closed 並標為內部政策而非 NVR。此構面已無扣分項 |
| 證據與可稽核性 | 15 | 9 | 13 | 頁碼 9／9 複驗相符、雜湊與位元組數相符、官方 URL、`WEB-LIVE-REFERENCE` 誠實標記。扣分：無自我識別列、欄位型別混用、取得條件未標 |
| 缺口揭露之完整性 | 15 | 12 | 12 | Translation Fidelity Tests 是大幅加分，「Cat2 無變更」之嚴格範圍限定也處理得好。扣分：Category 1／4／7 MRG 的缺口揭露被刪除 |
| **總計** | **100** | **86** | **93** | |
...
| 分數 | 項目 | 性質 |
|---:|---|---|
| 3 | 自我識別列、欄位拆分、取得條件、責任人欄（5.2–5.5） | **機械性，約半小時** |
| 2 | 加回 Cat 1／4／7 MRG 缺口揭露（5.1） | **機械性，一列** |
| 1 | CR3035 操作性後果寫進 MT204 列（第三節） | **一句話** |
| 1 | 取得 Additional NVR 與 MyStandards exact UG 後，關閉 Translation Fidelity Tests 中的兩項發布阻擋 | **取得受控文件，非寫作問題** |

**前三項（6 分）都是文字工作，一小時內可完成，之後本表可到 99。** 最後 1 分需要受控文件到位，而 v5 自己已經把它正確地標成阻擋項——這是文件能做到的最好狀態。

---

## 七、建議：本表可以定版了

這是第三輪（v3 → v4 → v5），每一輪都確實變好，而且 v4 與 v5 各更正了我的一項錯誤（52a 角色歸屬、Cover MPG 版本）。現在的情況與先前設計方案在 v5 時相同——**文件面已接近到頂，再往上只能靠取得受控文件。**

真正該往下走的是兩件事，都不是修這張表：

1. **Category 4／7 MRG。** MT4xx／MT7xx 是本專案主戰場，目前零一手覆核。以本表建立的方法（逐欄 status／option／NVR 覆核 ＋ 跨欄位規則 ＋ 角色所有權分類 ＋ 頁碼機械產生）套用過去即可，方法已成熟。
2. **Translation Portal / MyStandards exact rules。** Translation Fidelity Tests 的八個案例目前全部無法執行，其中兩個已標為發布阻擋。這是取得文件的問題，但它擋在發布前面。

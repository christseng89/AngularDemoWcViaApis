...
BASE_URL=https://staging.example.internal/api \
  qa/tests/mt2/run_mt2_139.sh
```

The default report is `qa/reports/latest/mt2/mt2-139-curl-results.json`. Override it with
...
Only paths declared in `qa/tests/mt2/final/case-endpoints.json` and ending in
...
```bash
node --test qa/tests/mt2/*.test.mjs
npx prettier --check qa/tests/mt2
npm run lint
npm run typecheck
```

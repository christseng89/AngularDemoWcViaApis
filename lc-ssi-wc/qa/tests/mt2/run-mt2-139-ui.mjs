...
const reportFile = path.resolve(
  workspace,
  process.env.MT2_QA_UI_REPORT ??
    "qa/reports/latest/mt2/mt2-139-ui-results.json",
);
const summaryFile = path.resolve(
  workspace,
  process.env.MT2_QA_UI_SUMMARY ??
    "qa/reports/latest/mt2/MT2_139_UI全量測試報告_ZH.md",
);
const evidenceDirectory = path.resolve(
  workspace,
  process.env.MT2_QA_UI_EVIDENCE ?? "qa/reports/latest/mt2/mt2-139-ui-evidence",
);
...
const prepared = await prepareCases({
  workbook,
  endpointsFile: path.join(workspace, "qa/tests/mt2/final/case-endpoints.json"),

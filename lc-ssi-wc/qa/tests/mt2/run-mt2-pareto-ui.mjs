...
if (runLabel && !/^[A-Za-z0-9_-]+$/.test(runLabel))
  throw new Error(
    "MT2_QA_RUN_LABEL must contain only letters, numbers, _ or -",
  );
...
const fixedReportPath = path.join(
  root,
  `qa/reports/latest/mt2/mt2-pareto-fixed-results${fileSuffix}.json`,
);
const randomReportPath = path.join(
  root,
  `qa/reports/latest/mt2/mt2-pareto-random-results${fileSuffix}.json`,
);
const aggregateReportPath = path.join(
  root,
  `qa/reports/latest/mt2/mt2-pareto-release-results${fileSuffix}.json`,
);
const summaryPath = path.join(
  root,
  `qa/reports/latest/mt2/MT2_第二輪QA_20_80_Pareto結果${titleSuffix}_ZH.md`,
);
...
const fixedExecution = run("qa/tests/mt2/run-mt2-139-ui.mjs", {
  MT2_QA_CASES: selection.fixedCaseIds.join(","),
  MT2_QA_UI_REPORT: relative(fixedReportPath),
  MT2_QA_UI_SUMMARY: `qa/reports/latest/mt2/MT2_第二輪QA_Pareto_Fixed結果${titleSuffix}_ZH.md`,
  MT2_QA_UI_EVIDENCE: `qa/reports/latest/mt2/mt2-pareto-fixed-evidence${fileSuffix}`,
});
const randomExecution = run(
  "qa/tests/mt2/run-mt2-resolver-exhaustive-random-ui.mjs",
  {
    ...
    MT2_QA_RANDOM_SUMMARY: `qa/reports/latest/mt2/MT2_第二輪QA_Pareto_Random結果${titleSuffix}_ZH.md`,
    MT2_QA_RANDOM_EVIDENCE: `qa/reports/latest/mt2/mt2-resolver-exhaustive-evidence${fileSuffix}`,
  },
);
...
const report = {
  ...
  replayCommand: runLabel
    ? `$env:MT2_QA_RUN_LABEL='${runLabel}'; $env:MT2_QA_RANDOM_SEED='${runtimeSeed}'; node qa/tests/mt2/run-mt2-pareto-ui.mjs`
    : "node qa/tests/mt2/run-mt2-pareto-ui.mjs",
};

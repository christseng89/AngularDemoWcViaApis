...
The complete TypeScript program and its tests are in [`governed-data-repair/`](governed-data-repair/):
...
```powershell
node --experimental-strip-types qa/FIX_DATA/governed-data-repair/cli.ts --output=tmp/governed-data-repair.json
```
...
```powershell
node --test --experimental-strip-types qa/FIX_DATA/governed-data-repair/*.test.ts
```

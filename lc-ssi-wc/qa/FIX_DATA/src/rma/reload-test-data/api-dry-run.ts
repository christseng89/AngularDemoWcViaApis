...
interface CanonicalSeed {
  ...
  readonly mappingSummary?: {
    readonly positiveContexts?: number;
    readonly negativeContexts?: number;
...
interface DataTypeReport {
  ...
  readonly unexpectedRejected: number;
  readonly errorCounts: Readonly<Record<string, number>>;
  readonly controlledOracle?: ControlledOracleReport;
}

interface ControlledOracleReport {
  readonly profileId: "MT347_DEMO_FIN_REFERENCE_ONLY_V1";
  readonly positiveExpected: number;
  readonly positiveAccepted: number;
  readonly negativeExpected: number;
  readonly negativeReasonMatched: number;
  readonly outOfScopeExpected: number;
  readonly outOfScopeSubmitted: number;
  readonly unexpectedPositiveRejection: number;
  readonly unexpectedNegativeAcceptance: number;
  readonly negativeReasonMismatch: number;
...
class ReloadTestDataApiDryRun {
  ...
  private async validateDataType(
    ...
    const controlled = {
      ...
      positiveAccepted: 0,
      negativeExpected: 0,
      negativeReasonMatched: 0,
      unexpectedPositiveRejection: 0,
      unexpectedNegativeAcceptance: 0,
      negativeReasonMismatch: 0,
    };
    let batchNumber = 0;
    for (const { offset, records: batch } of this.batches(records)) {
      ...
      for (const result of response.results) {
        ...
        const expected =
          dataType === "SSI"
            ? this.controlledExpectation(source.payload)
            : undefined;
        if (expected?.scope === "QA_POSITIVE") {
          controlled.positiveExpected += 1;
          if (result.status === "VALIDATED") {
            controlled.positiveAccepted += 1;
            continue;
          }
          controlled.unexpectedPositiveRejection += 1;
        } else if (expected?.scope === "QA_NEGATIVE") {
          controlled.negativeExpected += 1;
          if (result.status === "VALIDATED") {
            controlled.unexpectedNegativeAcceptance += 1;
          } else if (result.code === expected.reasonCode) {
            controlled.negativeReasonMatched += 1;
            expectedRejected += 1;
            continue;
          } else {
            controlled.negativeReasonMismatch += 1;
          }
    ...
    return {
      report: {
        ...
        rejected,
        expectedRejected,
        unexpectedRejected: errors.length,
        ...
        ...(dataType === "SSI"
          ? {
              controlledOracle: {
                profileId: "MT347_DEMO_FIN_REFERENCE_ONLY_V1" as const,
                ...controlled,
                outOfScopeExpected:
                  seed.mappingSummary?.outOfScopeOracleOnlyContexts ?? 0,
                outOfScopeSubmitted: 0,
                passed:
                  controlled.positiveExpected ===
                    controlled.positiveAccepted &&
                  controlled.negativeExpected ===
                    controlled.negativeReasonMatched &&
                  controlled.unexpectedPositiveRejection === 0 &&
                  controlled.unexpectedNegativeAcceptance === 0 &&
                  controlled.negativeReasonMismatch === 0,
              },
            }
          : {}),
      },
  ...
  private controlledExpectation(
    payload: Record<string, unknown>,
  ):
    | { readonly scope: "QA_POSITIVE" | "QA_NEGATIVE"; readonly reasonCode: string }
    | undefined {
    ...
    if (scope !== "QA_POSITIVE" && scope !== "QA_NEGATIVE") return undefined;
    return {
      scope,
      reasonCode: String(route["oracleReasonCode"] ?? ""),
    };
  }

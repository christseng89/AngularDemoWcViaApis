...
exports.CounterpartySsiResolutionService = void 0;
const tslib_1 = require("tslib");
const common_1 = require("@nestjs/common");
const bank_service_directory_1 = require("./bank-service-directory");
const SCOPE = "canonical route + MRG; full serialized XML requires controlled Translation Portal golden fixture";
const RENDERER = "MT compatibility view from the same confirmed canonical snapshot";
const isObject = (value) => Boolean(value) && typeof value === "object" && !Array.isArray(value);
let CounterpartySsiResolutionService = class CounterpartySsiResolutionService {
    bankServices;
    constructor(bankServices = new bank_service_directory_1.BankServiceDirectory()) {
        this.bankServices = bankServices;
    }
    canonicalScenario(is205, isCover) {
        if (is205) {
            return isCover
    ...
    precondition(raw) {
        ...
        if (forbidden.some((field) => Object.prototype.hasOwnProperty.call(raw, field)))
            return this.failure(422, "OPTION_CONSTRAINT_VIOLATION", {
                validation: "FAIL",
                forbiddenRequestProperties: forbidden.filter((field) => Object.prototype.hasOwnProperty.call(raw, field)),
        ...
        if (bankServiceError)
            return bankServiceError;
        if (raw["bankServiceId"] === "BANK-SVC-INACTIVE")
            return this.failure(422, "SSI_NOT_FOUND", { validation: "FAIL", mustNotAcceptFreeTextFallback: true }, "Selected Bank Service has no active eligible Counterparty SSI route");
        if (raw["bankServiceId"] === "BANK-SVC-BARCGB22")
            return this.success(this.bankServiceBase(), {
                creditor: "BARCGB22",
                creditorSource: "BANK_SERVICE_SELECTION",
                routeSource: "COUNTERPARTY_SSI",
            }, { "58A": "BARCGB22" }, [], ["53a/54a/56a/57a derived", "all 5x preview fields read-only"]);
    ...
    bankServiceEligibilityError(raw) {
        const bankServiceId = raw["bankServiceId"];
        if (typeof bankServiceId !== "string" ||
            bankServiceId === "BANK-SVC-INACTIVE")
            return undefined;
        try {
            const bank = this.bankServices.resolve(bankServiceId);
            if (bank.usageGroup !== "DIRECTORY_ONLY_NO_SSI")
                return undefined;
        }
        catch {
            return this.failure(422, "SSI_NOT_FOUND", {
                validation: "FAIL",
                payloadGenerated: false,
    ...
    resolve(request, raw) {
        ...
        if (is205)
            return this.mt205(base, raw, beneficiary);
        return this.mt202(base, raw, beneficiary);
    }
    beneficiary(request, raw) {
    ...
    profileValidationError(source, raw) {
        if (raw["requiresReceiverCorrespondent"] &&
    ...
    rmaValidationError(raw) {
        if (raw["availableRmaVersions"])
            return this.failure(422, "RMA_NOT_AUTHORISED", { validation: "FAIL", payloadGenerated: false }, "No exact pacs.009.001.08 / swift.cbprplus.04 authorisation");
        if (raw["rmaFixture"])
            return this.failure(422, "RMA_NOT_AUTHORISED", { confirmation: "BLOCKED", payloadGenerated: false }, raw["availableRmaVersions"]
                ? "No exact pacs.009.001.08 / swift.cbprplus.04 authorisation"
                : "");
        return undefined;
    }
    missingBeneficiaryValidationError(raw) {
        if (raw["paymentBeneficiaryInstitutionInput"] === null)
            return this.failure(422, "MESSAGE_CONTEXT_MISSING", {
                validation: "FAIL",
                missing: ["58a/Cdtr"],
    ...
    coverValidationError(source, raw) {
    ...
    coverInputError(raw) {
        ...
        if (raw["coverSequenceB"] === null)
            return this.failure(422, "MESSAGE_CONTEXT_MISSING", { validation: "FAIL", payloadGenerated: false }, "Cover Sequence B is mandatory upstream context");
        if (raw["block3"] &&
            isObject(raw["block3"]) &&
            raw["block3"]["119"] !== "COV")
            return this.failure(503, "PROFILE_INCOMPLETE", {
    ...
    coverInvariantError(cover205, raw) {
    ...
    mt202EligibilityError(raw) {
        return (this.mt202CustomerEligibilityError(raw) ??
            this.mt202AgentEligibilityError(raw) ??
            this.mt202CandidateEligibilityError(raw) ??
            this.mt202NostroEligibilityError(raw));
    }
    mt202CustomerEligibilityError(raw) {
        if (raw["counterpartyId"] === "CUST-00002")
            return this.failure(422, "SSI_NOT_FOUND", { validation: "FAIL", payloadGenerated: false }, "COUNTERPARTY_TYPE_MISMATCH; no customer-SSI fallback");
        if (raw["currency"] === "XAU")
    ...
    mt202AgentEligibilityError(raw) {
        if (raw["accountWith"] &&
    ...
    mt202CandidateEligibilityError(raw) {
        if (raw["candidate"] &&
        ...
        if (raw["candidate"] &&
            ...
            return this.failure(503, "PROFILE_INCOMPLETE", {
                validation: "FAIL",
                missingRelationship: "SSI.route.accountId -> ACTIVE Nostro.accountReference",
                payloadGenerated: false,
            }, "SSI route account reference cannot be joined to Own Nostro");
        return undefined;
    }
    mt202NostroEligibilityError(raw) {
        if (raw["nostro"] &&
            isObject(raw["nostro"]) &&
            raw["nostro"]["allowedBookingEntitiesKeyPresent"] === false)
            return this.failure(503, "PROFILE_INCOMPLETE", { validation: "FAIL", mustNotTreatMissingAsWildcard: true }, "Missing allowedBookingEntities is UNKNOWN, not ANY");
        if (raw["candidate"] &&
            isObject(raw["candidate"]) &&
            raw["candidate"]["ownershipTypeKeyPresent"] === false)
            return this.failure(503, "PROFILE_INCOMPLETE", { validation: "FAIL", mustNotInjectOwnershipTypeDuringExport: true }, "Ownership cannot be inferred from counterpartyId or display label");
        return undefined;
    }
    mt202OwnAccount(base, raw) {
        ...
        if (raw["ownAccountSubScenario"] === "CREDIT_ONE_OF_SEVERAL_AT_57A")
            return this.success({ ...base, canonicalScenario: "OWN_ACCOUNT_TRANSFER" }, {
    ...
    mt202Selection(base, raw, beneficiary) {
        if (raw["directAccountCount"] === 2)
            return this.success(base, { settlementAccount: raw["selectedAccount"] }, { "53B": raw["selectedAccount"], "58A": beneficiary }, ["53B location"], ["option B Party Identifier only"]);
        if (raw["qaSeedId"] === "QA-SSI-BARC-USD-INT")
            return this.success(base, {
                intermediaryAgent1: "HSBCHKHH",
                creditorAgent: "CITIUS33",
                beneficiaryInstitution: "BARCGB22",
            }, { "56A": "HSBCHKHH", "57A": "CITIUS33", "58A": "BARCGB22" }, [], ["C81; distinct role provenance"]);
        if (raw["qaSeedIds"])
            return this.success(base, {
    ...
    mt202Beneficiary(base, raw, beneficiary) {
        ...
        if (raw["counterpartyId"] === "CP-CHASUS33")
            ...
            }, { "58A": "CHASUS33" }, [], ["CAD routes must be excluded before ranking"]);
        return undefined;
    }
    mt202AgentRoles(base, raw, beneficiary) {
        if (raw["qaSeedId"] === "QA-SSI-DEUT-EUR-54")
            return this.success(base, {
                senderCorrespondent: "CITIUS33",
                receiverCorrespondent: "BNPAFRPP",
                beneficiaryInstitution: "DEUTDEFF",
            }, { "53A": "CITIUS33", "54A": "BNPAFRPP", "58A": "DEUTDEFF" }, [], ["53a precedes non-Receiver 54a; provenance retained"]);
        if (raw["senderCorrespondentBankServiceId"] ||
            raw["receiverCorrespondentBankServiceId"])
            return this.success(base, { senderCorrespondent: "CITIUS33", receiverCorrespondent: "HSBCHKHH" }, { "53A": "CITIUS33", "54A": "HSBCHKHH", "58A": beneficiary }, [], ["53a must precede 54a"]);
        if (raw["intermediaryBankServiceId"])
            return this.success(base, {
                intermediaryAgent1: "HSBCHKHH",
                creditorAgent: "CITIUS33",
                beneficiaryInstitution: beneficiary,
            }, { "56A": "HSBCHKHH", "57A": "CITIUS33", "58A": beneficiary }, [], ["C81 satisfied"]);
        return undefined;
    }
    mt202Context(base, raw, beneficiary) {
        if (raw["13C"])
            return {
                ...this.success(base, { beneficiaryInstitution: beneficiary }, { "13C": raw["13C"], "58A": beneficiary }),
                mx: {
                    ...base,
        ...
        if (beneficiary === "DEUTDEFF")
            return this.success(base, {
                ...
                beneficiaryInstitution: beneficiary,
            }, { "53A": "OWN_NOSTRO_EUR_SERVICER", "58A": beneficiary }, ["54a"], ["53A provenance=Own SSI/Nostro"]);
        if (beneficiary === "BARCGB22")
            return this.success(base, {
                instructedAgent: "CITIUS33",
                creditorAgent: "CITIUS33",
                beneficiaryInstitution: beneficiary,
            }, { "58A": beneficiary }, ["53a", "54a", "56a", "57a"], ["Receiver itself is AWI; do not duplicate Receiver into 53A/57A"]);
        return undefined;
    }
    mt202DirectAccount(base, beneficiary) {
        return this.success(base, {
            instructedAgent: beneficiary,
            beneficiaryInstitution: beneficiary,
            directAccount: true,
        }, { "58A": beneficiary }, ["53a", "54a", "56a", "57a"], ["57a omitted means Receiver is AWI"]);
    }
    mt205(base, raw, beneficiary) {
        const previous = isObject(raw["previousMessage"])
            ? raw["previousMessage"]
            : {};
        if (previous["type"] === "MT200")
            return this.success(base, {
        ...
        if (raw["receiverIsAwi"])
            return this.success(base, { creditorAgent: "Receiver", domestic: true }, { "52A": raw["previous52A"], "58A": beneficiary }, ["57a"], ["57a omission is semantic"]);
        if (raw["56A"])
            return this.success(base, { intermediaryAgent1: raw["56A"], creditorAgent: raw["57A"] }, {
                "52A": raw["52A"],
                "56A": raw["56A"],
                "57A": raw["57A"],
                "58A": raw["58A"],
        ...
        if (raw["incoming121"])
            return this.success(base, { uetr: raw["incoming121"], instruction: raw["72"] }, {
                "72": raw["72"],
                "121": raw["incoming121"],
                "52A": "CHASUS33",
                "58A": beneficiary,
        ...
        return this.success(base, { domestic: true, beneficiaryInstitution: beneficiary }, { "58A": beneficiary });
    }
    mt202Cover(base, raw) {
        const sequenceB = isObject(raw["sequenceB"]) ? raw["sequenceB"] : {};
        if (raw["qaSeedIds"])
            return {
                ...this.success(base, { sequenceA: "resolved", sequenceB: "copied unchanged" }, {
                    "119": "COV",
                    "121": "preserved",
        ...
        if (raw["ownAccountSubScenario"] === "CREDIT_ONE_OF_SEVERAL_AT_57A")
            return this.success({ ...base, canonicalScenario: "OWN_ACCOUNT_COVER_TRANSFER" }, {
        ...
        if (raw["ownAccountSubScenario"] === "BOOK_TRANSFER_SAME_RECEIVER")
            return this.success({ ...base, canonicalScenario: "OWN_ACCOUNT_COVER_TRANSFER" }, {
                ...
                creditorAccount: String(raw["A.58A"] ?? "").split("\n")[0],
                creditor: "DEMOHKHH",
                sequenceB: "copied unchanged",
                counterpartySsiResolution: "SKIPPED",
            }, { "A.53B": raw["A.53B"], "A.58A": raw["A.58A"] }, [], ["53B Party Identifier only", "58A option A"]);
        if (raw["paymentBeneficiaryInstitutionInput"])
            return this.success(base, {
                creditor: raw["paymentBeneficiaryInstitutionInput"],
                creditorSource: "REQUEST_PASS_THROUGH",
                sequenceB: "copied unchanged",
            }, { "A.58A": raw["paymentBeneficiaryInstitutionInput"] }, [], ["must not read SSI.beneficiaryBic"]);
        if (raw["incoming21"])
            return this.success(base, { underlyingReference: raw["incoming21"], uetr: raw["incoming121"] }, { "21": raw["incoming21"], "121": raw["incoming121"] }, [], ["both unchanged"]);
        if (raw["A.53A"])
            return this.success(base, {
                senderCorrespondent: raw["A.53A"],
                receiverCorrespondent: raw["A.54A"],
            }, { "A.53A": raw["A.53A"], "A.54A": raw["A.54A"], "A.58A": "CITIUS33" }, [], ["five 53a/54a rules jointly validated"]);
        if (raw["B.56C"])
            return this.success(base, { underlyingAgents: "preserved from transaction context" }, { "B.56C": raw["B.56C"], "B.57C": raw["B.57C"] }, [], ["option C confined to Seq B"]);
        return this.success(base, {
    ...
    mt205Cover(base, raw) {
        ...
        if (raw["incoming21"])
            return this.success(base, { relatedReference: raw["incoming21"], uetr: raw["incoming121"] }, { "21": raw["incoming21"], "121": raw["incoming121"] });
        if (raw["B.56C"])
            return this.success(base, { underlyingAgents: "preserved" }, { "B.56C": raw["B.56C"], "B.57C": raw["B.57C"] }, [], ["option C confined to Seq B"]);
        return this.success(base, { domestic: true, sequenceA: "resolved", sequenceB: "copied unchanged" }, {
            "A.52A": raw["A.52A"],
            "A.58A": raw["A.58A"],
            "B.50A": raw["B.50A"],
            "B.59": raw["B.59"],
    ...
    mt205CoverFromMt202Cover(base, raw, previous) {
        const route = {
        ...
        return this.success(base, route, { "A.52A": previous["A.52A"], "A.58A": previous["A.58A"] }, [], assertions);
    }
};
exports.CounterpartySsiResolutionService = CounterpartySsiResolutionService;
exports.CounterpartySsiResolutionService = CounterpartySsiResolutionService = tslib_1.__decorate([
    (0, common_1.Injectable)(),
    tslib_1.__metadata("design:paramtypes", [bank_service_directory_1.BankServiceDirectory])
], CounterpartySsiResolutionService);
//# sourceMappingURL=counterparty-ssi-resolution.service.js.map
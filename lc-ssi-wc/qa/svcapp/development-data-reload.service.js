...
const digest = (value) =>
  (0, node_crypto_1.createHash)("sha256").update(value).digest("hex");
const secureEquals = (actual, expected) =>
  (0, node_crypto_1.timingSafeEqual)(
...
const normalizedSeedValue = (value) =>
  (0, database_snapshot_identity_service_1.normalizeSqliteValue)(
    decodeSeedValue(value),
  );
const parseSeed = (raw) => {
  ...
  if (
    ...
    seed.identityMethod !==
      database_snapshot_identity_service_1.SQLITE_SNAPSHOT_IDENTITY_METHOD ||
    !seed.fixtureId ||
...
const seedIdentity = (seed) => {
  ...
  return (0, canonical_json_1.hashCanonical)({
    method: seed.identityMethod,
    tables,
  });
};
...
let DevelopmentDataReloadService = class DevelopmentDataReloadService {
  ...
  status() {
    const runtime = (0, runtime_environment_policy_1.resolveRuntimeEnvironment)(
      this.environment,
    );
    ...
    try {
      ...
      try {
        ...
        for (const name of tableNames)
          database.exec(
            `DELETE FROM ${(0, database_snapshot_identity_service_1.quoteSqliteIdentifier)(name)}`,
          );
        ...
        const identity = (0,
        database_snapshot_identity_service_1.databaseSnapshotIdentity)(
          database,
        );
  ...
  validateSchema(database, seed) {
    ...
    for (const name of expected) {
      const columns = database
        .prepare(
          `PRAGMA table_info(${(0, database_snapshot_identity_service_1.quoteSqliteIdentifier)(name)})`,
        )
        .all()
        .map((item) => item.name);
  ...
  insertTable(database, name, table) {
    const columns = table.columns
      .map(database_snapshot_identity_service_1.quoteSqliteIdentifier)
      .join(",");
    const placeholders = table.columns.map(() => "?").join(",");
    const insert = database.prepare(
      `INSERT INTO ${(0, database_snapshot_identity_service_1.quoteSqliteIdentifier)(name)} (${columns}) VALUES (${placeholders})`,
    );
    for (const row of table.rows) insert.run(...row.map(decodeSeedValue));
  }
  databasePath() {

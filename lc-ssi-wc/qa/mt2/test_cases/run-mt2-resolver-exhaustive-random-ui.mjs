...
const reportPath = path.resolve(
  ...
  process.env.MT2_QA_RANDOM_REPORT ??
    "qa/mt2/reports/mt2-resolver-exhaustive-random.json",
);
const summaryPath = path.resolve(
  root,
  process.env.MT2_QA_RANDOM_SUMMARY ??
    "qa/mt2/reports/MT2_RESOLVER_全量隨機瀏覽器測試_ZH.md",
);
const evidenceRoot = path.resolve(
  root,
  process.env.MT2_QA_RANDOM_EVIDENCE ??
    "qa/mt2/reports/mt2-resolver-exhaustive-evidence",
);
...
try {
  for (const item of orderedCases) {
    ...
    const result = {
      ...
      differences,
      replayCommand: `$env:MT2_QA_RANDOM_SEED='${seed}'; $env:MT2_QA_RANDOM_CASE='${item.caseId}'; node qa/mt2/test_cases/run-mt2-resolver-exhaustive-random-ui.mjs`,
    };
...
const report = {
  ...
  selectionFile: selectionFile
    ...
    : null,
  replayCommand: `$env:MT2_QA_RANDOM_SEED='${seed}'; node qa/mt2/test_cases/run-mt2-resolver-exhaustive-random-ui.mjs`,
  health: { portal: "HTTP_200", apiDiscovery: "HTTP_200" },
  snapshot: {
    source:
      "qa/mt2/mt2-final/fixtures/baselines/ssi-demo.consistent-snapshot.sqlite",
    sha256: sha256(
      fs.readFileSync(
        path.join(
          root,

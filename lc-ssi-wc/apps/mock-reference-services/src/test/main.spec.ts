...
describe("mock reference service contracts", () => {
  ...
  it("uses safe paging defaults for malformed values and an empty search", () => {
    const page = controller["bankPage"]("not-a-page", "not-a-size", "") as {
      page: number;
      pageSize: number;
      total: number;
      items: unknown[];
  ...
  it("returns active clearing systems without optional filters", () => {
    const result = controller["clearingSystems"]() as {
  ...
  it("rejects an unknown QA fixture even when governed QA mode is enabled", () => {
    process.env["ENABLE_QA_NEGATIVE_FIXTURES"] = "true";

    expect(() => controller["qaNegativeBankCounterparty"]("UNKNOWN-QA-FIXTURE"))
      .toThrow("QA negative fixture not found");
  });

...
import { createMaintenanceIndexActionAdapter } from "../../app/maintenance-index-action-policy";
import { assertMaintenanceServerPage } from "../../app/maintenance-index-server-page";
...
describe("maintenance index server-page contract", () => {
  ...
  it("renders shared maintenance server page items without post-pagination filtering", () => {
    const source = readFileSync(
      join(join(process.cwd(), "apps/ssi-portal/src/app"), "swift-data-feature", "swift-data-index.store.ts"),
      "utf8",
    );

...
jest.mock("../../app/ssi-maintenance-api.service", () => ({
  SsiMaintenanceApiService: class {},
}));
jest.mock("../../app/reference-lookup-api.service", () => ({
  ReferenceLookupApiService: class {},
}));
jest.mock("../../app/ssi-maintenance-feature/ssi-maintenance-session", () => ({
...
describe("portal routes", () => {
  ...
  it("keeps both SSI URLs and guarded loadComponent children in one lazy route scope", async () => {
    ...
    const { SsiMaintenanceSession } = await import("../../app/ssi-maintenance-feature/ssi-maintenance-session");
    const { SSI_MAKER_ROUTE_CONTEXT } = await import("../../app/ssi-maintenance-feature/ssi-maker-route-context");
  ...
  it("guards and lazy-loads SWIFT Data without eagerly importing it from Root", () => {
    ...
    expect(route?.canActivate).toHaveLength(1);
    const root = readFileSync(join(join(process.cwd(), "apps/ssi-portal/src/app"), "app.component.ts"), "utf8");
    const template = readFileSync(join(join(process.cwd(), "apps/ssi-portal/src/app"), "app.component.html"), "utf8");

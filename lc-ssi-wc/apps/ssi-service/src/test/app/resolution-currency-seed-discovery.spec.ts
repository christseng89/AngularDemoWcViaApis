...
describe("approved synthetic seed currency discovery", () => {
  ...
  beforeAll(() => {
    temporaryDirectory = mkdtempSync(
      join(process.cwd(), "tmp", "currency-seed-"),
    );
    ...
    for (const name of ["ssi", "ssi_applicability"]) {
      ...
      db.exec(
        `CREATE TABLE ${name} (${table.columns.map((column) => `${column} TEXT`).join(",")})`,
      );
      const insert = db.prepare(
        `INSERT INTO ${name} (${table.columns.join(",")}) VALUES (${table.columns.map(() => "?").join(",")})`,
      );
  ...
  it("discovers domain coverage from governed profiles, not a universal SSI DISTINCT", () => {
    ...
    const currencies = (domain: string) =>
      result.pairs
        .filter((pair) => pair.businessDomain === domain)
        .map((pair) => pair.currency);
    expect(currencies("PAYMENT")).toEqual([
      "AUD",
      "CAD",
      "CHF",
      "CNY",
    ...
    expect(currencies("TRADE_FINANCE")).toEqual([
      "EUR",
      "GBP",
      "HKD",
      "JPY",
  ...
  it("keeps narrow Payment currency SQL equivalent to the original eligibility query", () => {
    for (const profile of new PaymentResolutionPageDefinitionSource().coverageProfiles()) {
      ...
      const original = [
        ...new Set(
          repository
            .findPaymentCandidateBindings(query)
            .map(({ ssi }) => ssi.route.currency as string),
        ),
      ].sort();
      expect(repository.findPaymentResolutionCurrencies(query)).toEqual(
        original,
      );
    }
  });
  ...
  it("keeps MT202/205 and pacs.009 Core/COV candidate identities and settlement routes across ten governed currencies", () => {
    ...
    for (const sourceMessageType of [
      "MT202",
      "MT202COV",
      "MT205",
      "MT205COV",
      ...
      for (const currency of currencies) {
        ...
        for (const { ssi, applicability } of candidates) {
          ...
          seen.add(ssi.id);
          expect(ssi.route.sourceMessageTypes?.split(",")).toContain(
            sourceMessageType,
          );

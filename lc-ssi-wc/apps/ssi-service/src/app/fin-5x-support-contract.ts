...
export type Fin5xResolutionStatus =
...
const LEGAL_REASONS: Readonly<Record<string, readonly Fin5xReasonCode[]>> = {
...
export function assertFin5xSupportContract<T extends Fin5xSupportContract>(
  value: T & Record<string, unknown>,
): T {
  const legalReasons =
    LEGAL_REASONS[`${value.scopeStatus}|${value.resolutionStatus}`];
  const hasValue = !!value.suggestedValue?.trim();
  if (
    !legalReasons?.includes(value.reasonCode) ||

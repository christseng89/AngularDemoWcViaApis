...
- Canonical seed SHA 與 active Operational DB logical snapshot 是各自獨立的身分，不得因兩者資料內容不同（例如 Operational audit）而要求 SHA 相等。Final QA 必須分別驗證選用 seed 檔的受控 SHA，並在本輪開始時捕獲 Operational DB snapshot、於結束時證明仍為同一 snapshot；開始至結束發生 drift 才 fail closed。
...
一般未遵守規範仍依 `memory/governance/failed-task-retrospective-method-zh-v1.md` 的「第一次警告、第二次移出專案」處理；但下列兩類經事實與證據確認的重大違規，不適用第一次警告，立即停止任務、撤銷其專案角色、移出本專案工作小組並換人：
...
- `memory/ssi/swift-mt1xx-pacs008-ssi-v3.md`
- `memory/ssi/swift-mt2xx-pacs009-v2.md`
- `memory/ssi/swift-mt347-v2.md`
...
| v2.8.7  | 2026-09-15 | 新增所有 Message Family 的 SWIFT SSI 可見輸入標示規則：由 OAS／Page Parameters 提供 `SWIFT Tag+Option • Official Tag Description`，Generic UI 通用呈現；同步修正文件生效日並採外部 manifest 記錄 SHA 與具名 4-EYES                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Maker `/root`；Independent Checker 待重驗；詳見 `memory/governance/lc-ssi-wc-operating-model-zh-v2.manifest.json`                                           |
...
| v2.8.29 | 2026-09-25 | 依 Product Owner 裁定分離 canonical seed 與 Operational DB snapshot 身分：seed 驗自身受控 SHA；Final QA 捕獲當前 Operational snapshot 並要求整輪保持不變，不再把含不同業務／audit 資料的 Operational DB 強制等同 canonical seed SHA。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Governance Maker `/root`；待 Independent BA／QA／DBA 依新規則對最終 exact candidate commit 與捕獲 snapshot 重驗，未簽認前為 NOT_ACCEPTED                    |

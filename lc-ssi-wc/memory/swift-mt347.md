...
Scope: the 25 FIN messages currently displayed by the SSI Workbench Treasury and Trade Finance catalogue
...
- **Pending conversion**: no controlled customer/converter mapping contract and target ISO Usage Guideline has yet been supplied.
- **Product observation**: current code or UI behaviour, not a SWIFT rule.
...
| MT765 Guarantee/Standby Demand — p.354 | `56a A/B/D` O Intermediary; `57a A/B/D` O Account With Institution — p.355 | Both may use controlled routing data; the demand itself remains transaction narrative. |
...
## 8. MT↔MX conversion boundary

No reviewed source establishes a universal MT3xx/MT400/MT7xx-to-pacs mapping. The repository contains SR2026 CBPR+ guides for pacs.008.001.08, pacs.009.001.08, pacs.010.001.03 and camt.057.001.06, but that does not prove a target for these 25 FIN messages.

For customers using third-party MT↔MX conversion, each supported conversion requires a controlled contract containing:

- source MT type, sequence, field option, raw bytes/hash and MRG filename/SHA/release;
- target ISO message, usage profile, business service, XPath and target-guide filename/SHA/release;
- converter product/version, configuration hash and mapping-profile identity;
- canonical role/account identity and explicit presence/omission decisions;
- round-trip semantic tests for multi-leg, sequence and option preservation.

Do not infer a target from the UI business-function label, tag number or the existence of a pacs document. `pacs.009.001.12` strings currently present in portal business-function metadata are not backed by the controlled SR2026 `pacs.009.001.08` guides and must not be treated as verified conversion profiles.

## 9. Current implementation observations
...
1. Portal `verified` is computed from whether a generated scenario is marked `executable`; generated scenarios default to `executable: true`. Therefore `VERIFIED REFERENCE` is not an independent MRG or end-to-end execution result. UI should distinguish `MRG PROFILE VERIFIED`, `SSI SEMANTICS VERIFIED`, `CONVERSION PENDING` and `UAT EXECUTED`.
...
7. Option D/J suggestions require complete option-specific source data and provenance; BIC-only fallback fails closed.
8. A converter test is not PASS without a contracted target guide/profile and semantic round-trip evidence. FIN-only profile success must be reported separately.
...
When a standards release, field profile, SSI policy or converter changes:
...
4. Re-adjudicate reusable SSI versus transaction-context roles.
5. Revalidate each contracted MT↔MX mapping; never carry a mapping forward solely because the MT tag number is unchanged.

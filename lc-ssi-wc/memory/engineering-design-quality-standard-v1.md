...
- Each external candidate handoff must provide the exact base and candidate Git commits, same-commit four-eyes evidence, and a recoverable patch. Do not embed exact commits into controlled documents or manifests. Do not use `reset`, `checkout`, `stash`, `clean`, or an equivalent operation in a way that can overwrite or hide another owner's work.
- Four-eyes review is mandatory. Maker and Independent QA／Checker must verify the same exact candidate commit; authors cannot approve their own work.
...
workgroup completes
  ...
  -> PM verifies result, evidence, external candidate commit, tests, risks, and remaining ownership
...
- The workstream owner report must include scope／result, status, external base／candidate commits and changed files when applicable, tests／evidence, DB or other mutation status, remaining issues, and next owner.
...
| Reproducibility           | External base／candidate commits, ordered files, commands, environment, and non-Git evidence identities are reproducible |
| Defect severity           | Open Critical = 0; open High = 0                                                                           |
| Independent review        | Independent QA／Checker verifies the exact Maker candidate commit                                          |
...
Overall release authorization additionally requires every included defect／feature candidate to retain defect-level `PASS`, all cross-feature and end-to-end gates to pass, the complete release manifest and rollback／recovery evidence to match the externally recorded release candidate commit, and the authorized release approvers to record approval. Any changed artifact, failed gate, unresolved Critical／High defect, unexpected mutation, or candidate mismatch returns the overall gate to `NOT AUTHORIZED`.

...
The Index `SSI GENERATED TAGS` column follows the MT2 compact style and lists only governed MT Tag/Option families (`53a`–`57a`, with STP's exact uppercase option A where applicable). MT/MX prefixes and pacs.008 element names do not belong in this compact column. The controlled REMIT row is `APPROVED` for Product `FIN`, service `MT103_REMIT`, community `CBPR_PLUS`, MUG `MT103_REMIT_SR2026`, effective from `2026-01-01` through `9999-12-31`.
...
This demo renderer is intentionally limited to governed Option A SSI evidence. Any resolved MT projection outside the profile's `swiftMtRenderableOptions` fails closed with `PROFILE_INCOMPLETE`; it must never be rendered using the Option A `/account + BIC` layout.
...
| [Operating model](../governance/lc-ssi-wc-operating-model-zh-v2.md)                                            | Git-tracked path in the externally recorded exact candidate commit | Product boundary, controlled context, TDD, Page Parameters and Four-eyes governance.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
This v3 candidate supersedes v2 as the proposed active MT1 / pacs.008 knowledge entry point. Superseded v0.6 and v2 workspace files were removed; their history remains available through Git history and is not current implementation authority.

...
- Canonical seed identity and the active Operational DB logical snapshot are independent identities. QA must verify the selected seed file against its own controlled SHA, capture the current Operational DB snapshot at the start of a run, and prove the same captured snapshot remains in force at the end. It must not require an Operational DB containing different audit or business data to equal the canonical seed SHA.

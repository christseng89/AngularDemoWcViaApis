...
3. The MT1／pacs.008 business scope is already decided by `memory/swift-mt1xx-pacs008-v2.md` and `PO-SCOPE-MT1-20260916`; it is not an open Product Owner scope question. The catalogue generator must materialise the following governed Phase-1 rules from that normative source instead of hard-coding them in UI, API, seed, audit, or repair code:
   ...
   - `pacs.008.001.12` appearing in legacy fixtures or seed data is not authority for this scope. It must not coexist as a selectable catalogue entry, be accepted by manual API validation, be written by Load Data, be treated as supported by Audit, or be retained／written by Repair. Load Data and Repair record it as `IGNORED_OUT_OF_SSI_SCOPE`; manual ADD／EDIT rejects it.
...
4. Intersect each row with the supported catalogue.
   ...
   - Remove `pacs.008.001.12` as `IGNORED_OUT_OF_SSI_SCOPE`.
...
25. Given legacy fixture data containing `pacs.008.001.12`, separate executable tests prove that it cannot coexist as selectable in the catalogue, UI does not offer it, manual API ADD／EDIT rejects it, Load Data logs `IGNORED_OUT_OF_SSI_SCOPE` without storing it, Audit classifies it unsupported, and Repair removes it from the retained set and never writes it back; `.001.08` remains the governed entry.

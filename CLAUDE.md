...
- `docs/obsidian-balance-kb-v3.2/` — a generated Obsidian vault mirroring `lc-payment-wc/docs/
  ...
  ~700+-note, manually-curated structure with a per-note CONFIRMED/INFERRED/UNCLEAR/CONFLICT
  evidence-status label into a fully scripted, consolidated set (50 notes as of the rewrite — one
  canonical note per concept, cross-linked via Wiki links rather than duplicated) generated by
  `scripts/rebuild-obsidian-kb.mjs --write` and checked by `scripts/validate-obsidian-kb.mjs` (duplicate
  titles/bodies, broken Wiki links, missing cited `source_files`, and a >95% documentation-coverage gate
  read from `12-Traceability/Documentation Coverage.md`). Every note's frontmatter now carries `status`,
  `source_of_truth: source-code`, `source_revision` (the git short-hash the vault was generated at),
  `verified_date`, and `generated: true` instead of the old per-note evidence label; start from
  `00-Home/Home.md`, and see `00-Home/Vault Conventions.md` for the frontmatter/linking/regeneration
  rules before hand-editing a note (a hand edit survives until the next `--write` regenerates it from
  source). **Tracked in git**, unlike `lc-payment-wc/`'s own `obsidian-payment-kb/` — root `.gitignore`
  has an `obsidian-balance-kb*/` wildcard rule that would cover this directory, but that line is
  commented out, so all files in it are committed and `git status` will show them as modified whenever
  the vault regenerates. Don't assume this is gitignored without re-checking `.gitignore` first.

...
**Gate lifted (2026-08-24, Chris Tseng, direct named instruction in-session — not a BA-role document-drafting confirmation)**: `Maturity-Date-UI-Override-Business-Confirmation-Summary.md`'s own 確認記錄 table has been filled in accordingly. Engineering may now proceed on the full set above — `sightDate` definition, Mode A vs B, DP/DA routing, Import A6 accounting classification, Calendar Snapshot retention, the §4.4 Base Date Correction mechanism, and UI-Override questions 1-5 — without waiting for a separately-executed physical/meeting sign-off. **Exception, explicitly not covered by this lift**: UI-Override question 6 (whether the calculated Contractual Maturity Date / Operational Payment Date are displayed on screen at all) is a substantive design question, not a sign-off formality, and remains genuinely open — do not treat its "建議：要顯示" as decided.

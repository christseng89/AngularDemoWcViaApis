...
| 2026-08-23 | 收尾修正 | 9.5/10 | 修正本文件與 Revision-Spec 各自單獨維護、彼此對不上的靜態「日期」欄位（改成「初次/最後更新」雙欄位並指向各自的版本歷程表，不再單獨維護），並把本文件 Scope 段落過期的 `info.version: "1.16.0"` 加註說明（OAS 現為 1.19.0）。Revision-Spec 第 9 節新增的「子帳未結清時批次呼叫 A10/B6 預期失敗（409），非系統故障」這條補充，同步回填進 `Natural-Expiry-Scope-Decision-Request.md`（GAP-15 的權威回覆記錄），避免只讀決策請求文件、沒翻到 Revision-Spec 的人漏掉這個預期行為。本列本身即補記上一輪未留紀錄的落差 |

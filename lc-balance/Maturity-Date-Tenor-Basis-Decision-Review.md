...
| 「業務已核定」標示的實際確認層級（本版新增，v33 更正 Go-Live Blocker 分類） | 全文件 | **✅ 正式簽核已完成（2026-08-24）** | 否（v33 當時「是」，本次更正） | `Maturity-Date-UI-Override-Business-Confirmation-Summary.md`「確認記錄」表已由 Chris Tseng 具名填寫，`CLAUDE.md`「Tenor Basis / Risk Containment Gate」的實作擱置已正式解除——但**僅止於已有內容答案、只差簽核形式的項目**（`sightDate` 定義、Mode A vs Mode B、`AFTER_ACCEPTANCE`、UI 覆寫問題一～五）；下方 DP／DA 路由規則、Import A6 會計分類仍是**內容本身**未定義（非簽核問題），Calendar Snapshot 保存期限的簽核方也不是 TF Business 而是 Standing 團隊——這三項簽核解除不會替它們生出答案，見各自那一列 |

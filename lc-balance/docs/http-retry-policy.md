...
本文件是 Angular Balance UI 自动重试的现行操作规范。OAS 的 `x-client-retry-policy` 用于记录此 client policy；微服务本身不会替 caller 重试。
...
`scripts/generate-runtime-config.mjs` 在 `start`、`dev:all`、`test`、`build` 前读取 `.env`，产生 Angular 使用的型别安全设定。环境值改变后须重新启动或 build；直接执行 `ng serve` 前应先运行 `npm run generate:config`。

## Verification

Interceptor tests 覆盖暂时性 GET 失败后重试三次，以及 POST 失败不重试。OAS `1.42.1` 只新增 vendor-extension metadata，没有改变 endpoint、request、response 或 status contract。Channel OAS 保持 `1.9.0`。

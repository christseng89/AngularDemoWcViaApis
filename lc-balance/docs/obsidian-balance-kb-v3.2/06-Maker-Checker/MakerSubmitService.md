...
`submit()` 以 RxJS `defer` 包覆 dispatch，因此单笔与 compound shape 的同步例外也会成为保留 raw cause 的 `failed` outcome。A1-A11／B1-B7 共用同一 error boundary；HTTP 4xx business rejection 不再被 UI 误标为 `BAL-UI-UNEXPECTED`。

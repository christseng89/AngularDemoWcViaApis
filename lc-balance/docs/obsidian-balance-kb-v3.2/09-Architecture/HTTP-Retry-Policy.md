...
Balance UI只自動重試安全讀取：`GET`、`HEAD`、`OPTIONS`。專案根目錄`.env`預設最多重試3次、初始間隔250ms、上限2000ms，分別由`BALANCE_HTTP_RETRY_COUNT`、`BALANCE_HTTP_RETRY_INITIAL_DELAY_MS`及`BALANCE_HTTP_RETRY_MAX_DELAY_MS`控制。

...
**文件性質**：提案（Proposal），尚未包含完整測試程式碼。待您確認範疇與矩陣後，再產出正式的 Business Case Registry 條目（`businessCases.js`）或 curl 腳本。
...
| A10 | Close | **完全沒有覆蓋** | **完全沒有覆蓋** | **完全沒有覆蓋** |

### 1.2 Export Confirmed LC 現況

| Function | 名稱 | Sight | Usance |
|---|---|---|---|
| B1 | Confirm LC | ✅ export-case-1/6、shell 腳本 S01 | ✅ export-case-3/7（Seller's Usance）、shell 腳本 U01。export-case-2/4 目前用 `tenorType: 'BUYERS_USANCE'`，**2026-08-21 決議：Export 沒有 Buyer's Usance，需修正（見決策備忘錄決策 2，修正方向待與程式員共同確認）** |
| B2 | Amendment | 只以 `AMEND_INCREASE` 步驟穿插在 export-case-2/4/5 這種複合案例裡出現，**沒有獨立的 B2 案例**，也沒有像 import-case-5 那樣的負向（減額拒絕）案例 |
...
| import-case-11（可選） | A10 Close 資格門檻負向案例 | 刻意在 SG 或 Acceptance 尚未歸零時呼叫 Close，預期回傳資格不符錯誤（比照 import-case-5 的 `expectError: true` 慣例），驗證三個 Close 前置條件真的有被 enforce | **A10**（負向路徑） |
...
> 修正項目（非新增覆蓋，屬既有資料修正）：`export-case-2`／`export-case-4` 目前用 `tenorType: 'BUYERS_USANCE'`，需依決策備忘錄決策 2 修正。這不是單純換成 `SIGHT` 就結束——若改成 SIGHT，情境會跟 export-case-1／6 重複、失去原本想測的 B4 ACCEPT 複合流程價值，建議與程式員一起重新檢視這兩案的真實測試意圖（很可能應該改成 `SELLERS_USANCE`），此項列在決策備忘錄的行動項目 4，非本提案矩陣的一部分。
...
1. 第 3 節的範疇問題已定案（見上），第 4 節矩陣已依此收斂為 import-case-8～11、export-case-8～10 共 7 個新案例，另加 `export-case-2`／`4` 一項修正待辦。
2. 《Balance-Component-Business-Rule-Decisions-2026-08-21.md》裡的行動項目 1–3（A9 前端鎖定、後端 businessEventId 配對檢查、`BUYERS_USANCE` 於 Export 的拒絕／正規化）需要先落地，新案例的正確性才有意義——建議程式員先處理這三項，新測試案例的撰寫可以並行草擬，但驗證要排在規則落地之後。
3. 確認後，我會依此矩陣產出正式的 `businessCases.js` step-list 程式碼（或視您偏好改採 curl shell 腳本形式），並排入既有的「comments only，不直接改動應用程式原始碼」審閱模式——這份新增測試程式碼會先以建議／草稿形式交給您與程式員，而不是我直接寫入專案。

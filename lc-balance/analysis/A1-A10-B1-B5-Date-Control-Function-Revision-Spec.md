...
| 第八版 | 2026-08-23 | 使用者 | 指出 GAP-15 遺漏一環：`expiryDate` 只加了欄位，沒有任何查詢端點能篩選它，外部系統/UI 無法有效率找到候選 LC。新增第 0.1 節，比照本系統既有的 A10/B6 三層防線慣例（Discovery／Maker／Checker）設計：`close-eligible` 新增 `expiredBefore` 篩選參數（第 1 層，唯一做判斷的一層）；Maker Submit／Checker Release 各自新增純資訊性的 `triggeredByExpiry` 稽核欄位（第 2/3 層，只記錄不判斷，絕不能把 `expiryDate` 變成放行條件，延續第 6 節既有警語）。使用者確認：業務控制邏輯全系統一致要求三層檢查，這不是本案例的特例。（本輪之後，第 0–1、0.1、2、3 節（AMEND_EXPIRY／`documentPresentationDate` 檢查部分）、第 7 節已實作並驗證：三個測試套件全綠、OAS 更新至 v1.20.0、瀏覽器實測 A1 Issue／A2 Extend Expiry 全流程） |
| 第九版 | 2026-08-23 | 使用者 | 開始實作第 2/3 節 A6/B4 的 Calculated Maturity Date 時發現：`Business Day Convention` 這半句話微服務目前無法真正落地——沒有假日曆（GAP-15 已定案，排程/計日邏輯是外部系統職責，不是 Balance Component 自己管）。已發起 `Maturity-Date-Business-Day-Convention-Decision-Request.md`，第 2/3 節 A6/B4 兩列的「具體修正規格」標記為**🚧 待業務決定**，暫緩實作，直到有明確方向為止——其餘已實作部分不受影響 |
| 第十版（本版） | 2026-08-23 | 使用者 | **`Maturity-Date-Business-Day-Convention-Decision-Request.md` 已回覆——選項 (c)，但透過新增一個獨立的外部 Standing 微服務落地，GAP-15 分工本身不變**：Business Day Convention／假日曆邏輯仍不進 Balance Component，改成呼叫外部 Standing 服務的 `POST /business-days/adjust`。詳細業務規則、資料模型與 OAS 設計見 `analysis/maturity_date/`（`Standing_Microservice_Maturity_Date_OAS_Design.md` v2.10.0，19 輪審閱，9.9/10 Approved Design Baseline；`standing-calendar-service.oas.yaml`）。Balance Component 端職責不變——依 Tenor Basis／UCP 600 Art. 3 算出 `sourceDate`（候選 Maturity Date）；Standing 端接手判斷該日是否為營業日、依 Convention 順延/提前，回傳 `adjustedDate`（Operational Payment Date）與逐行事曆判定明細，且固定回傳 `contractualDateChanged: false`——Contractual Maturity Date 本身永遠不因這次呼叫而改變，只有「哪一天實際撥款/處理」會調整，與本文件既有「Maturity Date 仍是分類判斷基準」（見第七版第 4 節）的立場一致。**本輪僅記錄決策、解除 A6/B4 兩列的阻塞狀態，尚未實作**——第 2/3 節 A6/B4 兩列改標記為「✅ 決策已定案，待實作」，實作本身（含是否先用本地 Mock Server 對接、`maturityDateOverrideReason` 覆寫欄位、UI 唯讀顯示）留待下一輪排入 |
...
| **A6 Acceptance (Usance)** | YES，看 underlying presentation | **CREATE — 新增 Calculated Maturity Date（✅ 決策已定案，待實作，見下）** | ✅ **決策已定案（2026-08-23），待實作**——`Calculated = Acceptance Date + tenorDays + Business Day Convention` 的 Business Day Convention 半句，改由外部 Standing 微服務的 `POST /business-days/adjust` 負責（GAP-15 分工不變，Balance Component 仍不內建假日曆）；`sourceDate = Acceptance Date + tenorDays` 由 Balance Component 自算後送出，取回 `adjustedDate`（Operational Payment Date）與 `contractualDateChanged: false`。詳見 `analysis/maturity_date/`、`Maturity-Date-Business-Day-Convention-Decision-Request.md`（決策記錄）。UI 唯讀顯示、「手動調整」+ 填寫理由覆寫（`maturityDateOverrideReason?: string \| null`）、`tenorRouting.ts` 一致性檢查維持不變這幾點本身不受影響 |
...
| **B4 Honour / Acceptance** | YES，不是簡單 Block | **CREATE / CONFIRM — 新增 Calculated Maturity Date（✅ 決策已定案，待實作，見下）** | ✅ **決策已定案（2026-08-23），待實作**——同 A6，Usance 分支（`movementType: 'ACCEPT'`）的 Calculated Maturity Date 改呼叫外部 Standing 微服務的 `POST /business-days/adjust`，詳見 `analysis/maturity_date/`、`Maturity-Date-Business-Day-Convention-Decision-Request.md`；Sight 分支（`movementType: 'HONOUR'`）不受影響（無 Acceptance 產生），本來就不需要這個邏輯 |

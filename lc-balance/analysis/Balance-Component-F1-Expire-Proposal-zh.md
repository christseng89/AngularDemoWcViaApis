...
## 二十、business-days-mock Fail-closed 行事曆範圍防護——BA 複驗發現的缺口已修復（2026-08-26，BA 已驗證修復屬實）

使用者訊息「這部分改好了 檢查一下代碼」所指的，即 §18.4／§19 記錄的「沒有行事曆資料涵蓋範圍檢查」
這個非阻擋缺口。BA 透過檔案時間戳掃描發現 `microservices/business-days-mock/` 新增了
`test/server.test.js`（15 個測試）與 `package.json` 的 `jest`/`supertest` devDependencies，判斷這是
工程team針對此缺口的修復，逐一核對如下。

### 20.1　程式碼核對——`server.js` 新增的 fail-closed 防護

`CALENDAR_MIN_DATE`／`CALENDAR_MAX_DATE` 兩個常數改為**從 `calendar.json` 自己的 `holidays[]`
陣列動態算出**（`Math.min`/`Math.max` 年份，非寫死常數）——目前資料為 2026-2028，算出
`2026-01-01`／`2028-12-31`，涵蓋範圍隨行事曆資料本身自動延展，符合 §18.4 原本點名的「應動態算出、
不應寫死」精神（雖然 §18.4 原文沒有明講這個要求，但這是比寫死常數更穩健的作法）。

防護分兩層,BA 逐一讀過程式碼確認：

1. **輸入日期本身超出範圍**（`app.post` handler 內）：`date < CALENDAR_MIN_DATE || date >
   CALENDAR_MAX_DATE` → 直接 `422 CALENDAR_RANGE_EXCEEDED`,不進入 `addBusinessDays()` 迴圈。
2. **輸入日期在範圍內,但往前走 N 個營業日會走出範圍**（`addBusinessDays()` 迴圈內）：每走一天先檢查
   `dateStr > CALENDAR_MAX_DATE`,一旦超出立刻回傳 `outOfRange: true`,由 handler 轉成同一個
   `422 CALENDAR_RANGE_EXCEEDED`（訊息文字不同，說明是「walk 過程中」超出而非「輸入本身」超出）。

**特別確認邊界情況未被誤傷**（避免防護矯枉過正，把合法查詢也擋掉）：`businessDays: 0` 且輸入日期剛好
是涵蓋範圍最後一天（`2028-12-31`）時，迴圈條件 `remaining > 0` 一開始就不成立，不執行任何一次迭代，
直接回傳輸入日期本身──不會誤觸防護。

### 20.2　BA 未只讀程式碼、也未只信任 README 更新後的新宣稱，而是實際執行測試套件

`microservices/business-days-mock/README.md` 這輪新增一段「Fail-closed 行事曆範圍防護（2026-08-26，
BA 複驗發現後修復）」的說明，文字描述與程式碼相符──但比照本次複查全程的方法論，BA 沒有只讀這段
新文字就採信，而是實際執行了工程team新增的 Jest 測試套件（`npx jest`）：
...
15 項測試全數通過，涵蓋原有的週末/假日跳過邏輯（迴歸測試，確認新防護沒有破壞既有行為）、輸入驗證
（無效日期格式、負數/非整數 `businessDays`），以及新增的 5 項 fail-closed 範圍防護測試：範圍後
（2029）、範圍前（2025）、輸入在範圍內但 walk 會超出範圍（`2028-12-30` +5 營業日）、範圍最後一天
`businessDays: 0` 仍正常成功（不誤觸防護）、以及一個「真正可解但貼近邊界」的 walk 仍正常成功
（`2028-12-27` +2 營業日）　

### 20.3　BA 獨立驗算邊界測試案例的日期數學，非採信測試斷言本身

比照本次複查全程「不採信文件/測試自我宣稱」的方法論，BA 另外獨立算過兩個邊界測試案例的星期幾，
確認測試斷言的期望值本身正確（而不只是「程式碼跟測試互相一致」但兩者一起錯）：

- `2028-12-27` 為星期三。+2 個營業日：12-28（四，營業日,剩1）→12-29（五，營業日,剩0），得出
  `2028-12-29`，與測試斷言一致（12 月無假日，只需算平日）。
- `2028-12-30` 為星期六。+5 個營業日往前走會先遇 12-31（日，跳過）,接著要走到 2029-01-01，
  已超出 `CALENDAR_MAX_DATE`（`2028-12-31`），觸發 `outOfRange` → `422`，與測試斷言一致。

### 20.4　結論

**§18.4／§19 記錄的「沒有行事曆資料涵蓋範圍檢查」缺口，經 BA 複驗，確認已修復，且修復方式正確、
測試涵蓋完整（含邊界情況）、未破壞既有行為（原有測試全數迴歸通過）。** 驗證方式為：讀
`server.js` 原始程式碼＋實際執行 Jest 測試套件（非採信 README 自述）＋BA 獨立驗算兩個邊界案例的
日期數學（非採信測試斷言本身）。`business-days-mock` 至此沒有已知缺口。仍待處理的既有事項不變：
BAL-001 零驗證缺口（獨立 Gate Condition）、Phase 2 尚未實際接進 `microservices/balance-component/`
本身。
